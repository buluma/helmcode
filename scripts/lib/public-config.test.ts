...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.HELMCODE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.HELMCODE_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.HELMCODE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.HELMCODE_RELAY_URL).toBeUndefined();
    expect(env.VITE_HELMCODE_RELAY_URL).toBeUndefined();
    expect(env.HELMCODE_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.HELMCODE_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.HELMCODE_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.HELMCODE_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.HELMCODE_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.HELMCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "HELMCODE_CLERK_PUBLISHABLE_KEY=pk_root\nHELMCODE_CLERK_JWT_TEMPLATE=template_root\nHELMCODE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nHELMCODE_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "HELMCODE_CLERK_PUBLISHABLE_KEY=pk_local\nHELMCODE_CLERK_JWT_TEMPLATE=template_local\nHELMCODE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nHELMCODE_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).HELMCODE_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          HELMCODE_CLERK_PUBLISHABLE_KEY: "pk_ci",
          HELMCODE_CLERK_JWT_TEMPLATE: "template_ci",
          HELMCODE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          HELMCODE_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      HELMCODE_RELAY_URL: "https://ci.example.test",
      VITE_HELMCODE_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        HELMCODE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_HELMCODE_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          HELMCODE_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          HELMCODE_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          HELMCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          HELMCODE_RELAY_URL: "https://relay.example.test",
          HELMCODE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          HELMCODE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          HELMCODE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      HELMCODE_RELAY_URL: "https://relay.example.test",
      VITE_HELMCODE_RELAY_URL: "https://relay.example.test",
      HELMCODE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      HELMCODE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      HELMCODE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "helmcode-public-config-"));
