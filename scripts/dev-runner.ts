...
import { resolveGitWorktreePath, resolveWorktreeHelmCodeHome } from "@helmcode/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@helmcode/shared/hostProcess";
...
export const DEFAULT_HELMCODE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".helmcode"),
);

const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@helmcode/contracts",
    "--filter=@helmcode/web",
    "--filter=helmcode",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=helmcode", "dev"],
  "dev:web": ["run", "--filter=@helmcode/web", "dev"],
  "dev:desktop": ["run", "--filter=@helmcode/desktop", "--filter=@helmcode/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`HELMCODE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("HELMCODE_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("HELMCODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("HELMCODE_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "HELMCODE_PORT_OFFSET",
    ...
    return Effect.succeed({
      ...
      source: `HELMCODE_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric HELMCODE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed HELMCODE_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_HELMCODE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly helmcodeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .helmcode > ambient HELMCODE_HOME) is resolved
    // by the caller; an unset helmcodeHome here genuinely means "use the default".
    const configuredBaseDir = helmcodeHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.HELMCODE_HOME = resolvedBaseDir;
    } else {
      delete output.HELMCODE_HOME;
    }

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed helmcode service (an
    // agent working inside Helm Code), these leak through and the child server
    // fails startup with "The service launcher started a different helmcode version"
    // (serviceLauncherClient.ts resolveStartup).
    delete output.HELMCODE_SERVICE_LAUNCHER_CONTEXT;
    delete output.HELMCODE_BOOT_SERVICE_UNIT;

    if (!isDesktopMode) {
      output.HELMCODE_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.HELMCODE_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.HELMCODE_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.HELMCODE_PORT = String(serverPort);
      ...
      delete output.HELMCODE_SINGLE_ORIGIN_DEV;
      delete output.HELMCODE_MODE;
      delete output.HELMCODE_NO_BROWSER;
      delete output.HELMCODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.HELMCODE_HOST = host;
    }

    if (!isDesktopMode) {
      output.HELMCODE_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.HELMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.HELMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.HELMCODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.HELMCODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.HELMCODE_MODE = "web";
      delete output.HELMCODE_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`HELMCODE_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["HELMCODE_PORT_OFFSET", "HELMCODE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.helmcode` — see @helmcode/shared/devHome for why this must
    // outrank an ambient HELMCODE_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreeHelmCodeHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedHelmCodeHome =
      (input.helmcodeHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.HELMCODE_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      helmcodeHome: resolvedHelmCodeHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.HELMCODE_HOME ?? (yield* DEFAULT_HELMCODE_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.HELMCODE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.HELMCODE_DEV_ALLOWED_ORIGINS = [
            env.HELMCODE_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so HELMCODE_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.HELMCODE_BUNDLED_DEV === undefined) {
            env.HELMCODE_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  helmcodeHome: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Helm Code data directory; runtime state is stored under userdata (equivalent to HELMCODE_HOME). Inside a git worktree this defaults to that worktree's own .helmcode so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to HELMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("HELMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to HELMCODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("HELMCODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to HELMCODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("HELMCODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to HELMCODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("HELMCODE_PORT")),
  ),
