...
const ANDROID_PACKAGE = "com.helmcode.helmcode";
const APP_SCHEME = "helmcode";
const IOS_READY_FILENAME = "HelmCodeShowcaseReadyScene";
const SERVER_HOST = "0.0.0.0";
const IOS_SIMULATOR_ARCH = NodeProcess.arch === "arm64" ? "arm64" : "x86_64";
const IOS_APP_PATH = NodePath.join(
  MOBILE_ROOT,
  ".showcase/ios-derived-data/Build/Products/Debug-iphonesimulator/HelmCode.app",
);
...
const MOBILE_BUILD_ENV = {
  ...
  HELMCODE_SHOWCASE_CAPTURE_BUILD: "1",
  JAVA_HOME:
...
Usage:
  pnpm --filter @helmcode/mobile screenshots [options]
...
async function buildIos(): Promise<string> {
  ...
  await runCommand(
    ...
    [
      "-workspace",
      NodePath.join(MOBILE_ROOT, "ios/HelmCode.xcworkspace"),
      "-scheme",
      "HelmCode",
      "-configuration",
      "Debug",
...
async function captureIos(
  ...
  const scenePath = NodePath.join(
    ...
    "Library/Caches/HelmCodeShowcaseScene",
  );
  const readyPath = NodePath.join(
    await iosAppContainer(simulator.udid),
    "Library/Caches",
...
async function writeAndroidShowcaseScene(serial: string, scene: ShowcaseScene): Promise<void> {
  await runAdb(serial, [
    "shell",
    `run-as ${ANDROID_PACKAGE} sh -c 'mkdir -p files && rm -f files/helmcode-showcase-ready && printf %s ${scene} > files/helmcode-showcase-scene'`,
  ]);
}
...
async function main(): Promise<void> {
  ...
  const showcaseRootDir = await NodeFSP.mkdtemp(
    NodePath.join(NodeOS.tmpdir(), "helmcode-mobile-showcase-"),
  );
