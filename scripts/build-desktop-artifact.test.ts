...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
      ...
      resolveDesktopRuntimeDependencies(
        {
          ...
          "@helmcode/contracts": "workspace:*",
          "@helmcode/shared": "workspace:*",
          "@helmcode/ssh": "workspace:*",
          "@helmcode/tailscale": "workspace:*",
          effect: "catalog:",
          electron: "41.5.0",
        },
        {
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      ...
      HELMCODE_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });

    assert.deepStrictEqual(configuration, {
      appId: "com.helmcode.helmcode",
      teamId: "ABC1234567",
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      HELMCODE_APPLE_TEAM_ID: "ABC1234567",
      HELMCODE_MACOS_PROVISIONING_PROFILE: "/tmp/helmcode.provisionprofile",
      HELMCODE_CLERK_PASSKEY_RP_DOMAINS:
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const invalidDomainError = captureError({
      HELMCODE_APPLE_TEAM_ID: "ABC1234567",
      HELMCODE_MACOS_PROVISIONING_PROFILE: "/tmp/helmcode.provisionprofile",
      HELMCODE_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          HELMCODE_APPLE_TEAM_ID: "ABC1234567",
          HELMCODE_MACOS_PROVISIONING_PROFILE: "/tmp/helmcode.provisionprofile",
          HELMCODE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
