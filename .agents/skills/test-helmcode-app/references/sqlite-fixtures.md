...
Load this reference only when inspecting or seeding local HelmCode state directly.
...
```bash
node apps/server/scripts/helmcode-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/helmcode-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/helmcode-seed.sql
```
