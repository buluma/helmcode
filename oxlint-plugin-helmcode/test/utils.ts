...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "helmcode-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-helmcode", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "helmcode", specifier: pluginPath }],
