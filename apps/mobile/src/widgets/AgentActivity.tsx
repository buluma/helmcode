...
export function AgentActivity(
  ...
  const deepLink =
    deepLinkRow && deepLinkRow.deepLink.startsWith("/") && !deepLinkRow.deepLink.startsWith("//")
      ? `helmcode://${deepLinkRow.deepLink.slice(1)}`
      : null;
  ...
  // The branded HelmCode mark. `assetName` resolves the template image set bundled in
  ...
  const renderLogo = (height: number, color: string) => (
    <HStack modifiers={[frame({ width: height * 1.5, height }), foregroundStyle(color)]}>
      <Image assetName="HelmCodeMark" modifiers={[resizable()]} />
    </HStack>
  );
