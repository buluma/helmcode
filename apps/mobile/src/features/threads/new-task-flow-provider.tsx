...
} from "@helmcode/shared/threadEnvMode";
...
import { EnvironmentProject } from "@helmcode/client-runtime/state/shell";
import { type VcsRef } from "@helmcode/client-runtime/state/vcs";
import {
...
export function NewTaskFlowProvider(props: React.PropsWithChildren) {
  ...
  // checked-in helmcode.json, then the server's configured default.
  const helmcodeProjectFileQuery = useEnvironmentQuery(
    selectedProject !== null && selectedProject.workspaceRoot !== ""
      ? projectEnvironment.readFile({
          environmentId: selectedProject.environmentId,
          input: { cwd: selectedProject.workspaceRoot, relativePath: HELMCODE_PROJECT_FILE_NAME },
        })
      : null,
  );
  const helmcodeProjectFileData = helmcodeProjectFileQuery.data as ProjectReadFileResult | null;
  const helmcodeProjectFileDefaultMode = useMemo(() => {
    if (helmcodeProjectFileData === null || helmcodeProjectFileData.truncated) return null;
    return parseHelmCodeProjectFile(helmcodeProjectFileData.contents)?.defaultThreadEnvMode ?? null;
  }, [helmcodeProjectFileData]);
  const defaultWorkspaceMode: WorkspaceMode = resolveDefaultThreadEnvMode({
    projectSetting: selectedProject?.defaultThreadEnvMode,
    projectFile: helmcodeProjectFileDefaultMode,
  ...
  // the frozen interim value beats the helmcode.json default once it loads.
  const defaultWorkspaceModeSettled = isDefaultThreadEnvModeSettled({
    explicitMode: selectedProjectDraft.workspaceSelection?.mode,
    projectSetting: selectedProject?.defaultThreadEnvMode,
    projectFilePending: helmcodeProjectFileQuery.isPending,
  });
