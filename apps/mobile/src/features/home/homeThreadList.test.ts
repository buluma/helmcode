...
import { threadSearchMatchKey } from "@helmcode/client-runtime/state/thread-search";
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@helmcode/contracts";
...
describe("buildHomeThreadGroups", () => {
  it("builds one v2 scope for the same repository across environments", () => {
    ...
    expect(scopes[0]?.title).toBe("helmcode");
  ...
  it("routes stale duplicate project refs through the canonical repository group", () => {
    ...
    const local = makeProject({
      ...
      environmentId: localEnvironmentId,
      title: "helmcode",
      workspaceRoot: "/workspaces/helmcode",
      repositoryIdentity,
    });
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    ...
    const repositoryIdentity = {
      ...
      locator: {
        source: "git-remote" as const,
        remoteName: "origin",
        remoteUrl: "git@github.com:helmcode/helmcode.git",
      },
      provider: "github",
      owner: "helmcode",
      name: "helmcode",
      displayName: "Helm Code",
      rootPath: "/workspaces/helmcode",
    };
    const projects = [
      makeProject({
        environmentId,
        ...
        title: "Web",
        workspaceRoot: "/workspaces/helmcode/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/helmcode/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
  ...
  it("does not apply the recency window while searching", () => {
    ...
    const group = buildGroups([project], threads, { searchQuery: "Helm Code" })[0];
