import type { ProjectEntry } from "@helmcode/contracts";
    ...
    >
      ...
      <Text
        className={cn(
          ...
          props.selected
            ? "font-helmcode-bold text-foreground"
            : "font-helmcode-medium text-foreground-secondary",
        )}
        numberOfLines={1}
      >
        {node.name}
      ...
      {node.kind === "directory" ? (
        <Text className="text-2xs font-helmcode-medium text-foreground-tertiary">
  ...
  if (props.error && props.entries.length === 0) {
    return (
      <View className="flex-1 bg-sheet px-4 py-5">
        <Text className="text-sm font-helmcode-bold text-foreground">Files unavailable</Text>
      ...
      ListEmptyComponent={
        <View className="px-4 py-5">
          {props.isPending ? (
            ...
            <>
              <Text className="text-sm font-helmcode-bold text-foreground">No files found</Text>
