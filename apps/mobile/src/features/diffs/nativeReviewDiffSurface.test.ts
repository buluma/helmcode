...
describe("resolveNativeReviewDiffView", () => {
  ...
  it("returns the payload bridge when the native review diff view is installed", async () => {
    ...
    expect(expoMocks.requireNativeView).toHaveBeenCalledWith("HelmCodeReviewDiffSurface");
  });
  ...
  it("does not fall back to stale legacy native review diff view names", async () => {
    globalThis.expo = {
      getViewConfig: vi.fn().mockImplementation((moduleName: string) => {
        if (moduleName === "HelmCodeReviewDiffView") {
  ...
  it("returns null when the view manager cannot be required", async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        nativeModuleName: "HelmCodeReviewDiffSurface",
        cause,
      }),
    );
  ...
  it("recognizes registration races for the installed native view name", async () => {
    ...
    expect(
      isPendingNativeViewRegistration(
        new Error("Unable to find the 'HelmCodeReviewDiffSurface' view for this native tag"),
      ),
    ).toBe(true);
    expect(
      isPendingNativeViewRegistration(
        new Error("Unable to find the 'HelmCodeReviewDiffView' view for this native tag"),
      ),
    ).toBe(false);
    expect(
      isPendingNativeViewRegistration(
        new Error(
          "Unable to find the class expo.modules.helmcodereviewdiff.HelmCodeReviewDiffView view with tag 1150",
        ),
      ),
    ).toBe(true);
  });
