...
public final class HelmCodeKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("HelmCodeKeyboardCommands")

    View(HelmCodeKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: HelmCodeKeyboardCommandsView, commands: [String]) in
...
public final class HelmCodeKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.helmcodeFirstResponder == nil else { return }
...
private extension UIView {
  var helmcodeFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.helmcodeFirstResponder { return responder }
    }
    return nil
  }
}
