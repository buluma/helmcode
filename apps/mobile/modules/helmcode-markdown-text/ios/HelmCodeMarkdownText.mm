...
#import "HelmCodeMarkdownTextShadowNode.h"
#import "HelmCodeMarkdownTextComponentDescriptor.h"
...
static void HelmCodeMarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<HelmCodeMarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void HelmCodeMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<HelmCodeMarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = HelmCodeMarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        HelmCodeMarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
...
@protocol HelmCodeMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface HelmCodeMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<HelmCodeMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<HelmCodeMarkdownOutsideTapTarget>)target;

@end

static const void *HelmCodeMarkdownOutsideTapCoordinatorKey =
    &HelmCodeMarkdownOutsideTapCoordinatorKey;

@implementation HelmCodeMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<HelmCodeMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<HelmCodeMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, HelmCodeMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<HelmCodeMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static HelmCodeMarkdownOutsideTapCoordinator *
HelmCodeMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  HelmCodeMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, HelmCodeMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[HelmCodeMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        HelmCodeMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface HelmCodeMarkdownText () <RCTHelmCodeMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface HelmCodeMarkdownText () <HelmCodeMarkdownOutsideTapTarget>
@end

@implementation HelmCodeMarkdownText {
  UIView * _view;
  UITextView * _textView;
  HelmCodeMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<HelmCodeMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const HelmCodeMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    HelmCodeMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, HelmCodeMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [HelmCodeMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<HelmCodeMarkdownTextProps const>(_props);
  ...
  HelmCodeMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  HelmCodeMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::HelmCodeMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::HelmCodeMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<HelmCodeMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<HelmCodeMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<HelmCodeMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == HelmCodeMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == HelmCodeMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == HelmCodeMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == HelmCodeMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const HelmCodeMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (HelmCodeMarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[HelmCodeMarkdownTextRun class]]) {
      continue;
    }

    HelmCodeMarkdownTextRun* textChild = (HelmCodeMarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::HelmCodeMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::HelmCodeMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> HelmCodeMarkdownTextCls(void)
{
  return HelmCodeMarkdownText.class;
}
