...
static constexpr auto FileAttachmentNativeIdPrefix = "helmcode-file:";
static constexpr auto SkillAttachmentNativeIdPrefix = "helmcode-skill:";
...
HelmCodeMarkdownTextShadowNode::HelmCodeMarkdownTextShadowNode(
   const ShadowNode& sourceShadowNode,
   const ShadowNodeFragment& fragment
) : ConcreteViewShadowNode(sourceShadowNode, fragment) {
};

Size HelmCodeMarkdownTextShadowNode::measureContent(
  ...
  const LayoutConstraints& layoutConstraints) const {
    ...
    auto paragraphStyleRanges = std::vector<HelmCodeMarkdownTextParagraphStyleRange>{};
    auto attachmentRanges = std::vector<HelmCodeMarkdownTextAttachmentRange>{};
    size_t utf16Offset = 0;
    const auto &children = getChildren();
    for (size_t i = 0; i < children.size(); i++) {
      const auto child = children[i].get();
      if (auto textViewChild = dynamic_cast<const HelmCodeMarkdownTextRunShadowNode *>(child)) {
        ...
        if (props.fontStyle == HelmCodeMarkdownTextRunFontStyle::Italic) {
          textAttributes.fontStyle = FontStyle::Italic;
        } else {
          textAttributes.fontStyle = FontStyle::Normal;
        }

        if (props.fontWeight == HelmCodeMarkdownTextRunFontWeight::Bold) {
          textAttributes.fontWeight = FontWeight::Bold;
        } else if (props.fontWeight == HelmCodeMarkdownTextRunFontWeight::UltraLight) {
          textAttributes.fontWeight = FontWeight::UltraLight;
        } else if (props.fontWeight == HelmCodeMarkdownTextRunFontWeight::Light) {
          textAttributes.fontWeight = FontWeight::Light;
        } else if (props.fontWeight == HelmCodeMarkdownTextRunFontWeight::Medium) {
          textAttributes.fontWeight = FontWeight::Medium;
        } else if (props.fontWeight == HelmCodeMarkdownTextRunFontWeight::Semibold) {
          textAttributes.fontWeight = FontWeight::Semibold;
        } else if (props.fontWeight == HelmCodeMarkdownTextRunFontWeight::Heavy) {
          textAttributes.fontWeight = FontWeight::Heavy;
        } else {
          textAttributes.fontWeight = FontWeight::Regular;
        }

        if (props.textDecorationLine == HelmCodeMarkdownTextRunTextDecorationLine::LineThrough) {
          textAttributes.textDecorationLineType = TextDecorationLineType::Strikethrough;
        } else if (props.textDecorationLine == HelmCodeMarkdownTextRunTextDecorationLine::Underline) {
        ...
        if (props.textDecorationStyle == HelmCodeMarkdownTextRunTextDecorationStyle::Solid) {
          textAttributes.textDecorationStyle = TextDecorationStyle::Solid;
        } else if (props.textDecorationStyle == HelmCodeMarkdownTextRunTextDecorationStyle::Dotted) {
          textAttributes.textDecorationStyle = TextDecorationStyle::Dotted;
        } else if (props.textDecorationStyle == HelmCodeMarkdownTextRunTextDecorationStyle::Dashed) {
          textAttributes.textDecorationStyle = TextDecorationStyle::Dashed;
        } else if (props.textDecorationStyle == HelmCodeMarkdownTextRunTextDecorationStyle::Double) {
          textAttributes.textDecorationStyle = TextDecorationStyle::Double;
        }

        if (props.textAlign == HelmCodeMarkdownTextRunTextAlign::Left) {
          textAttributes.alignment = TextAlignment::Left;
        } else if (props.textAlign == HelmCodeMarkdownTextRunTextAlign::Right) {
          textAttributes.alignment = TextAlignment::Right;
        } else if (props.textAlign == HelmCodeMarkdownTextRunTextAlign::Center) {
          textAttributes.alignment = TextAlignment::Center;
        } else if (props.textAlign == HelmCodeMarkdownTextRunTextAlign::Justify) {
          textAttributes.alignment = TextAlignment::Justified;
        } else if (props.textAlign == HelmCodeMarkdownTextRunTextAlign::Auto) {
        ...
        if (hasParagraphStyle) {
          paragraphStyleRanges.push_back(HelmCodeMarkdownTextParagraphStyleRange{
              utf16Offset,
              fragmentLength,
        ...
        if (props.nativeId.rfind(FileAttachmentNativeIdPrefix, 0) == 0 && fragmentLength > 0) {
          attachmentRanges.push_back(HelmCodeMarkdownTextAttachmentRange{
              utf16Offset,
              1,
    ...
    if (baseProps.ellipsizeMode == HelmCodeMarkdownTextEllipsizeMode::Head) {
      textContainer.lineBreakMode = NSLineBreakByTruncatingHead;
    } else if (baseProps.ellipsizeMode == HelmCodeMarkdownTextEllipsizeMode::Middle) {
      textContainer.lineBreakMode = NSLineBreakByTruncatingMiddle;
    } else if (baseProps.ellipsizeMode == HelmCodeMarkdownTextEllipsizeMode::Tail) {
...
void HelmCodeMarkdownTextShadowNode::layout(LayoutContext layoutContext) {
  ensureUnsealed();
  setStateData(HelmCodeMarkdownTextStateReal{
    _attributedString,
