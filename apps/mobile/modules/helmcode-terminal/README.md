# HelmCode Mobile Terminal Native Module
...
fork, with HelmCode's iOS 16 compatibility patch applied. `HelmCodeTerminalView` owns a `libghostty` surface and
...
```bash
apps/mobile/modules/helmcode-terminal/scripts/build-libghostty-ios16.sh
```
...
```bash
apps/mobile/modules/helmcode-terminal/scripts/build-libghostty-android.sh
```
