// This file mostly exists because we want dev mode to say "Helm Code (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Helm Code (Dev)" : "Helm Code (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.helmcode.helmcode.dev.${devBundleIdSuffix || "local"}`
  : "com.helmcode.helmcode";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["helmcode-dev"] : ["helmcode"];
...
// oxlint-disable-next-line helmcode/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["HELMCODE_PORT", environment.HELMCODE_PORT],
    ["HELMCODE_HOME", environment.HELMCODE_HOME],
    ["HELMCODE_COMMIT_HASH", environment.HELMCODE_COMMIT_HASH],
    ["HELMCODE_OTLP_TRACES_URL", environment.HELMCODE_OTLP_TRACES_URL],
    ["HELMCODE_OTLP_EXPORT_INTERVAL_MS", environment.HELMCODE_OTLP_EXPORT_INTERVAL_MS],
    ["HELMCODE_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --helmcode-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Helm Code name.
