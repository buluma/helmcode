...
>()("@helmcode/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = [
  ...
  "HELMCODE_NO_BROWSER",
  "HELMCODE_HOST",
  "HELMCODE_DESKTOP_WS_URL",
  "HELMCODE_DESKTOP_LAN_ACCESS",
  "HELMCODE_DESKTOP_LAN_HOST",
  "HELMCODE_DESKTOP_HTTPS_ENDPOINTS",
  "HELMCODE_TAILSCALE_SERVE",
  "HELMCODE_TAILSCALE_SERVE_PORT",
] as const;
...
function resourceMonitorBinaryName(platform: NodeJS.Platform): string {
  return platform === "win32" ? "helmcode-resource-monitor.exe" : "helmcode-resource-monitor";
}
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    const bootstrap = {
      ...
      helmcodeHome: environment.baseDir,
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's HELMCODE_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  const bootstrap = {
    ...
    noBrowser: true,
    port: input.port,
    // Omit helmcodeHome so the Linux backend uses its own home dir instead of
  ...
  // Build an explicit copy of process.env minus HELMCODE_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.helmcode via
  // /mnt/c, which means both backends read/write the same database and
  // their env-ids collide).
  const parentEnvWithoutHelmCodeHome: Record<string, string | undefined> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "HELMCODE_HOME") continue;
    parentEnvWithoutHelmCodeHome[key] = value;
  }
  const wslEnv = mergeWslEnv(parentEnvWithoutHelmCodeHome.WSLENV, forwardedEnvNames);
  ...
  const baseConfig = {
    ...
    cwd: environment.backendCwd,
    env: {
      ...parentEnvWithoutHelmCodeHome,
      ...backendChildEnvPatch(),
      ...forwardedEnv,
      ...(wslEnv !== undefined ? { WSLENV: wslEnv } : {}),
    },
    // env is already a complete process.env minus HELMCODE_HOME; pass it
