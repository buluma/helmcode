...
const OVERLAY_ATTRIBUTE = "data-helmcode-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--helmcode-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--helmcode-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--helmcode-radius": theme.radius,
    "--helmcode-background": theme.background,
    "--helmcode-foreground": theme.foreground,
    "--helmcode-popover": theme.popover,
    "--helmcode-popover-foreground": theme.popoverForeground,
    "--helmcode-primary": theme.primary,
    "--helmcode-primary-foreground": theme.primaryForeground,
    "--helmcode-muted": theme.muted,
    "--helmcode-muted-foreground": theme.mutedForeground,
    "--helmcode-accent": theme.accent,
    "--helmcode-accent-foreground": theme.accentForeground,
    "--helmcode-border": theme.border,
    "--helmcode-input": theme.input,
    "--helmcode-ring": theme.ring,
    "--helmcode-font-sans": theme.fontSans,
    "--helmcode-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-helmcode-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--helmcode-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-helmcode-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
