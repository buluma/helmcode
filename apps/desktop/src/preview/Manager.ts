...
import { normalizePreviewUrl } from "@helmcode/shared/preview";
...
const makeNativeOperations = Effect.fn("PreviewManager.makeOperations")(function* (
  ...
  /**
   * Pushes a tab's committed mute onto whichever guest it currently owns,
   * reading both at call time so an older snapshot can never roll back a
   * mute action that landed after it. Mirrors how zoom is restored onto a
   * fresh guest attach in `registerWebviewUnlocked`.
   */
   ...
   * Shared by the `audio-state-changed` handler and the post-attach
   * reconcile so both drop values from a guest the tab no longer owns, and
   * both skip unchanged values: Chromium re-emits per media element, and
   * republishing would cost an IPC push per element rather than per real
   * transition.
   */
  ...
  const ensurePlaywrightInjected = Effect.fn("PreviewManager.ensurePlaywrightInjected")(function* (
    ...
    const installed = yield* evaluateWithDebugger<boolean>(
      tabId,
      send,
      "Boolean(globalThis.__helmcodePlaywrightInjected)",
      true,
    );
    if (installed) return;
  ...
  const attachListeners = Effect.fn("PreviewManager.attachListeners")(function* (
    ...
    const install = Effect.fn("PreviewManager.installWebContentsListeners")(function* () {
      ...
      yield* Ref.update(attachedRef, (attached) =>
        replaceMap(attached, (copy) => {
          copy.set(wc.id, { attachmentId, cancelFaviconCapture, scope });
        }),
      );
    });
  ...
  const registerWebviewUnlocked = Effect.fn("PreviewManager.registerWebviewUnlocked")(function* (
    ...
    // A replacement guest attaches unmuted, so reassert the tab's committed
    // mute before it is published rather than letting it emit audio the user
    // already silenced. Reconciled again after attach, below, the same way a
    // race with a concurrent setAudioMuted is handled there.
  ...
  const setAudioMuted = Effect.fn("PreviewManager.setAudioMuted")(function* (
    ...
    // committed value rather than this call's argument. Two overlapping
    // toggles would otherwise be free to commit in one order and reach
    // Chromium in the other, leaving the icon disagreeing with the guest.
  ...
  const resolveClickPoint = Effect.fn("PreviewManager.resolveClickPoint")(function* (
    ...
    >(
      ...
      `(() => {
          try {
            const injected = globalThis.__helmcodePlaywrightInjected;
  ...
  const typeIntoAutomationTarget = Effect.fn("PreviewManager.typeIntoAutomationTarget")(function* (
    ...
    >(
      ...
      `(() => {
          try {
            const element = ${locatorJson ? `(() => { const injected = globalThis.__helmcodePlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, true); })()` : "document.activeElement"};
            if (!element) return { notFound: true };
            const textControl =
  ...
  const performAutomationScroll = Effect.fn("PreviewManager.performAutomationScroll")(function* (
    ...
    >(
      ...
      `(() => {
        try {
          const target = ${locatorJson ? `(() => { const injected = globalThis.__helmcodePlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, true); })()` : "window"};
  ...
  const performAutomationWaitFor = Effect.fn("PreviewManager.performAutomationWaitFor")(function* (
    ...
    while ((yield* currentMillis) <= deadline) {
      ...
      >(
        ...
        `(() => {
              try {
                const selectorMatched = ${locatorJson ? `(() => { const injected = globalThis.__helmcodePlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, false) !== null; })()` : "true"};
                const textMatched = ${
...
>()("@helmcode/desktop/preview/Manager/PreviewManager") {}
