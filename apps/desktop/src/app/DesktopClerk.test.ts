...
const makeDesktopClerkLayer = (isDevelopment = true, events: string[] = []) => {
  const environment = DesktopEnvironment.DesktopEnvironment.of({
    stateDir: "/tmp/helmcode-state",
    isDevelopment,
    appDataDirectory: "/tmp/app-data",
    userDataDirName: isDevelopment ? "helmcode-dev" : "helmcode",
    legacyUserDataDirName: isDevelopment ? "Helm Code (Dev)" : "Helm Code (Alpha)",
...
describe("DesktopClerk", () => {
        ...
        [
          {
            ...
            passkeys: true,
            renderer: { scheme: "helmcode-dev", host: "app" },
          },
        ],
      ]);
  ...
  it.effect("preserves bridge initialization failures", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(error.stateDir, "/tmp/helmcode-state");
      assert.equal(error.isDevelopment, true);
      assert.strictEqual(error.cause, cause);
      assert.equal(
        error.message,
        'Failed to initialize the desktop Clerk bridge for state directory "/tmp/helmcode-state" (development: true).',
      );
    });
  });
  ...
  it.effect("preserves bridge cleanup failures", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(
          error.message,
          'Failed to clean up the desktop Clerk bridge for state directory "/tmp/helmcode-state" (development: false).',
        );
      }
    });
  });
  ...
  it.each([
    { isDevelopment: true, scheme: "helmcode-dev" },
    { isDevelopment: false, scheme: "helmcode" },
