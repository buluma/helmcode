...
const AppPackageMetadata = Schema.Struct({
  helmcodeCommitHash: Schema.optional(Schema.String),
});
...
>()("@helmcode/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.helmcodeCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
