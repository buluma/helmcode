...
import * as SshAuth from "@helmcode/ssh/auth";
import { discoverSshHosts } from "@helmcode/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@helmcode/ssh/errors";
import * as SshTunnel from "@helmcode/ssh/tunnel";
...
>()("@helmcode/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliPackageSpec?: () => string;
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemoteHelmCodeRunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Helm Code window is not available for SSH authentication.";
      break;
