import type { DesktopSshPasswordPromptRequest } from "@helmcode/contracts";
import { DesktopSshPasswordPromptResolutionInputSchema } from "@helmcode/contracts";
import type { SshPasswordRequest } from "@helmcode/ssh/auth";
...
export class DesktopSshPromptWindowUnavailableError extends Schema.TaggedErrorClass<DesktopSshPromptWindowUnavailableError>()(
  ...
  override get message(): string {
    ...
    return `Helm Code window is unavailable during ${this.stage} for SSH authentication to ${this.destination} (request: ${request}).`;
  }
}
...
>()("@helmcode/desktop/ssh/DesktopSshPasswordPrompts") {}
