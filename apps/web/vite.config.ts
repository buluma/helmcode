...
import { DEV_PROXIED_PATH_PREFIXES } from "@helmcode/shared/devProxy";
...
const isSingleOriginDev = process.env.HELMCODE_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_HELMCODE_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.HELMCODE_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: HELMCODE_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); HELMCODE_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.HELMCODE_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // HELMCODE_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.HELMCODE_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.HELMCODE_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_HELMCODE_RELAY_URL": JSON.stringify(configuredRelayUrl),
