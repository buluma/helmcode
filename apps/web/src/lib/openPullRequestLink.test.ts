...
describe("parseChangeRequestUrl", () => {
  it("reads a GitHub pull request", () => {
    expect(parseChangeRequestUrl("https://github.com/HelmCode/HelmCode/pull/123")).toEqual({
      host: "github.com",
  ...
  it("reads a GitLab merge request, nested groups and all", () => {
    expect(
      parseChangeRequestUrl("https://gitlab.com/helmcode/platform/helmcode/-/merge_requests/42"),
    ).toEqual({
      host: "gitlab.com",
      repository: "helmcode/platform/helmcode",
      number: 42,
    });
  });
  ...
  it("reads both Azure DevOps URL forms, keeping `_git` in the repository path", () => {
    expect(
      parseChangeRequestUrl("https://dev.azure.com/acme/platform/_git/helmcode/pullrequest/17"),
    ).toEqual({
      host: "dev.azure.com",
      repository: "acme/platform/_git/helmcode",
      number: 17,
    });
    expect(
      parseChangeRequestUrl("https://acme.visualstudio.com/platform/_git/helmcode/pullrequest/17"),
    ).toEqual({
      host: "acme.visualstudio.com",
      repository: "platform/_git/helmcode",
      number: 17,
    });
  });
  ...
  it("claims nothing it cannot be sure of, so the link goes to the browser", () => {
    for (const link of [
      "https://github.com/helmcode/helmcode/issues/123",
      "https://github.com/helmcode/helmcode/commit/0a1b2c3",
      "https://github.com/helmcode/helmcode",
      "https://github.com/helmcode/helmcode/pull/abc",
      "https://gitlab.com/helmcode/helmcode/-/snippets/12",
      "https://gitlab.com/helmcode/helmcode/-/issues/12",
      ...
      "javascript:alert(1)//github.com/helmcode/helmcode/pull/1",
      "not a url",
    ]) {
...
describe("findProjectForChangeRequest", () => {
  ...
  it("matches a nested GitLab group by the whole path below the host", () => {
    ...
    // two-segment owner/name form would look for `helmcode/helmcode` and find nothing.
    const projects = [
      project({
        canonicalKey: "gitlab.com/helmcode/platform/helmcode",
        provider: "gitlab",
        displayName: "helmcode/platform/helmcode",
        owner: "helmcode",
        name: "helmcode",
      }),
    ];
