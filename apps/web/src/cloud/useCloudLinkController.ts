...
/**
 * Drives the primary environment's HelmCode Connect link. HelmCode Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message = cause instanceof Error ? cause.message : "Could not update HelmCode Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[helmcode-connect] Could not update HelmCode Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update HelmCode Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn HelmCode Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to HelmCode Connect before enabling this."));
        return false;
      }
