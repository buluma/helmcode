...
import { EnvironmentRegistry } from "@helmcode/client-runtime/connection";
import { request, runStream } from "@helmcode/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@helmcode/client-runtime/rpc";
import { ManagedRelay } from "@helmcode/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Helm Code cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Helm Code cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "HELMCODE_RELAY_URL is not configured.",
      });
    }
...
// independently of HelmCode Connect while clients reach the environment out of band.
