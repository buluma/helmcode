...
import { UnifiedSettings } from "@helmcode/contracts/settings";
import { ReviewCommentContextSchema, type ReviewCommentContext } from "./reviewCommentContext";
const isRuntimeMode = Schema.is(RuntimeMode);
const isProviderDriverKind = Schema.is(ProviderDriverKind);
const isReviewCommentContext = Schema.is(ReviewCommentContextSchema);

export const COMPOSER_DRAFT_STORAGE_KEY = "helmcode:composer-drafts:v1";
...
export function composerFileMatchesReattachMarker(
  ...
  return (
    marker.name === file.name &&
    marker.sizeBytes === file.sizeBytes &&
    marker.mimeType === file.mimeType
  );
}
