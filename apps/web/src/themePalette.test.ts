...
describe("theme files", () => {
  it("derives a readable palette from extreme simple-editor colors", () => {
    ...
    // Status colors fall back to Helm Code's standard red and amber rather than
  ...
  it("canonicalizes the explicitly exported theme", () => {
    const serialized = serializeThemeFile({
      ...HELMCODE_CHAT_THEME,
      colors: { ...HELMCODE_CHAT_THEME.colors, accent: "hsl(263 70% 58%)" },
    });
    expect(JSON.parse(serialized)).toMatchObject({
      version: THEME_FILE_VERSION,
      id: HELMCODE_CHAT_THEME.id,
      name: HELMCODE_CHAT_THEME.label,
      appearance: "light",
  ...
  it("suppresses sidebar artwork during a live custom-theme preview", () => {
    ...
    applyThemeColorPreview(HELMCODE_CHAT_THEME.colors, "light");
  ...
  it("keeps optional light and dark palettes under one theme id", () => {
    ...
    expect(getThemeModes(HELMCODE_CHAT_THEME)).toEqual(["light", "dark"]);
    expect(resolveThemeAppearance(HELMCODE_CHAT_THEME.id, true, true)).toBe("dark");
    expect(resolveDesktopTheme(HELMCODE_CHAT_THEME.id, true)).toBe("system");
    expect(resolveThemeAppearance(HELMCODE_CHAT_THEME.id, false, false, "dark")).toBe("dark");
    expect(resolveDesktopTheme(HELMCODE_CHAT_THEME.id, false, "dark")).toBe("dark");
  ...
  it("keeps the Helm Code Chat palette faithful and readable", () => {
    expectThemeColors(HELMCODE_CHAT_THEME.colors, {
      canvas: "#fdf7fd",
      chrome: "#fdf7fd",
    ...
    expectThemeColors(HELMCODE_CHAT_THEME.variants!.dark!, {
      canvas: "#1f1a24",
      chrome: "#1f1a24",
      surface: "#29232d",
    ...
    for (const mode of ["light", "dark"] as const) {
      const colors = getThemeColorsForMode(HELMCODE_CHAT_THEME, mode)!;
      expect(contrastRatio(colors.text, colors.canvas)).toBeGreaterThanOrEqual(7);
      expect(contrastRatio(colors.textMuted, colors.canvas)).toBeGreaterThanOrEqual(4.5);
      expect(contrastRatio(colors.messageForeground, colors.messageSurface)).toBeGreaterThanOrEqual(
        4.5,
  ...
  it("includes the dual-mode maintainer themes", () => {
    for (const theme of [HELMCODE_CHAT_THEME, GROVE_THEME, OCEAN_THEME, EMBER_THEME, IRIS_THEME]) {
...
describe("stored theme preferences", () => {
  ...
  it("resolves the legacy helmcode-chat-dark preference to dark Helm Code Chat", () => {
    expect(getThemeDefinition("helmcode-chat-dark")).toBe(HELMCODE_CHAT_THEME);
    expect(getThemePreferenceMode("helmcode-chat-dark")).toBe("dark");
    expect(resolveThemeAppearance("helmcode-chat-dark", true, false)).toBe("dark");
    expect(resolveDesktopTheme("helmcode-chat-dark", false)).toBe("dark");
    expect(isKnownThemePreference("helmcode-chat-dark")).toBe(true);
  });

  it("resolves legacy helmcode-prefixed ids onto the renamed themes", () => {
    for (const [legacy, theme] of [
      ["helmcode-grove", GROVE_THEME],
      ["helmcode-ocean", OCEAN_THEME],
      ["helmcode-ember", EMBER_THEME],
      ["helmcode-iris", IRIS_THEME],
    ] as const) {
  ...
  it("recognizes only preferences the runtime can render", () => {
    for (const preference of ["light", "dark", "system", HELMCODE_CHAT_THEME.id, GROVE_THEME.id]) {
