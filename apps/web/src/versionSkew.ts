import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@helmcode/contracts";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "helmcode:version-mismatch-dismissals:v1";
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same Helm Code version.",
  };
}
...
export function manualServerUpdateCommand(targetVersion: string): string {
  return `npx helmcode@${targetVersion}`;
}
...
export function serverUpdateGuidance(
  ...
  switch (capability) {
    ...
    case "desktop-managed":
      return `The ${serverLabel} is run by the Helm Code desktop app on its machine — update the desktop app there to sync them.`;
    default:
