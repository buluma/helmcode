...
const THEME_STORAGE_KEY = "helmcode:theme";
...
describe("index.html boot script", () => {
  const parityCases: ReadonlyArray<{
    ...
    {
      name: "Helm Code Chat follows a dark OS",
      storage: { [THEME_STORAGE_KEY]: "helmcode-chat", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "an explicit global dark mode applies to Helm Code Chat",
      storage: {
        [THEME_STORAGE_KEY]: "helmcode-chat",
    ...
    {
      name: "a legacy helmcode-grove preference resolves through the alias",
      storage: { [THEME_STORAGE_KEY]: "helmcode-grove", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "legacy helmcode-chat-dark resolves to dark Helm Code Chat",
      storage: { [THEME_STORAGE_KEY]: "helmcode-chat-dark" },
      prefersDark: true,
    },
    {
  ...
  it("marks built-in and custom themes on the document element", () => {
    ...
    expect(chat.themeId).toBe("helmcode-chat");
  ...
  it("applies the matching half of an automatic mix to the splash", () => {
    const storage = {
      ...
      "helmcode:theme-halves:v1": JSON.stringify({ dark: GROVE_THEME.id }),
    };
    ...
    expect(light.themeId).toBe("helmcode-chat");
    expect(light.bootVariables["--boot-background"]).toBe(
      getThemeColorsForMode(HELMCODE_CHAT_THEME, "light")!.canvas,
    );
  });
  ...
  it("resolves a legacy-prefixed mix half onto the renamed theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "helmcode-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "helmcode:theme-halves:v1": JSON.stringify({ dark: "helmcode-grove" }),
      },
      prefersDark: true,
    });
    expect(boot.isDark).toBe(true);
  ...
  it("ignores a mix half that names an unknown theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "helmcode-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "helmcode:theme-halves:v1": JSON.stringify({ dark: "gone-theme" }),
      },
      prefersDark: true,
    });
    expect(boot.themeId).toBe("helmcode-chat");
    expect(boot.isDark).toBe(true);
  });
