...
function categoryLabel(category: ResourceTelemetryProcessCategory): string {
  switch (category) {
    ...
    case "resource-monitor":
      return "Monitor";
    case "unknown-helmcode":
      return "HelmCode process";
  }
}
...
export function ResourceTelemetryDiagnostics() {
  ...
  const allHelmCode = snapshot?.groups.allHelmCode;
        ...
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03),0_8px_30px_rgb(0_0_0/0.035)]">
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-border/55 md:grid-cols-3">
            <IconStat
              ...
              label="Current CPU"
              value={allHelmCode ? `${allHelmCode.currentCpuPercent.toFixed(1)}%` : "..."}
              detail={allHelmCode ? `${formatCpuTime(allHelmCode.cpuTimeMs)} observed CPU time` : undefined}
            />
            <IconStat
              icon={<MemoryStickIcon className="size-3.5" />}
              label="Resident memory"
              value={allHelmCode ? formatBytes(allHelmCode.currentRssBytes) : "..."}
              detail={
                allHelmCode ? `${formatBytes(allHelmCode.peakRssBytes)} combined process peaks` : undefined
              }
            />
            <IconStat
              icon={<ActivityIcon className="size-3.5" />}
              label="Process count"
              value={allHelmCode ? String(allHelmCode.processCount) : "..."}
              detail={
                allHelmCode ? `${allHelmCode.processStarts} starts · ${allHelmCode.processExits} exits` : undefined
              }
            />
            <IconStat
              icon={<HardDriveIcon className="size-3.5" />}
              label="Read throughput"
              value={allHelmCode ? formatRate(allHelmCode.ioReadBytesPerSecond) : "..."}
              detail={allHelmCode ? `${formatBytes(allHelmCode.ioReadBytes)} observed` : undefined}
            />
            <IconStat
              icon={<DatabaseIcon className="size-3.5" />}
              label="Write throughput"
              value={allHelmCode ? formatRate(allHelmCode.ioWriteBytesPerSecond) : "..."}
              detail={allHelmCode ? `${formatBytes(allHelmCode.ioWriteBytes)} observed` : undefined}
              tone={
                allHelmCode && allHelmCode.ioWriteBytesPerSecond >= 10 * 1_024 * 1_024
                  ? "danger"
                  : allHelmCode && allHelmCode.ioWriteBytesPerSecond >= 1_024 * 1_024
                    ? "warning"
                    : "default"
              }
            />
      ...
      >
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03)]">
          <div className="bg-muted/15 px-4 py-3 text-[11px] leading-relaxed text-muted-foreground sm:px-5">
            ...
            counters identify known HelmCode operations so process spikes can be correlated with specific
