...
import { scopeProjectRef } from "@helmcode/client-runtime/environment";
...
import type {
  ...
  HelmCodeProjectFileScript,
  ThreadEnvMode,
} from "@helmcode/contracts";
import { resolveEnvModeLabel } from "../BranchToolbar.logic";
import { createModelSelection } from "@helmcode/shared/model";
import { DEFAULT_RESOLVED_KEYBINDINGS } from "@helmcode/shared/keybindings";
...
import { useHelmCodeProjectFileState } from "../../hooks/useHelmCodeProjectFileScripts";
...
function ProjectDetail({ group }: { group: SidebarProjectSnapshot }) {
  ...
  const helmcodeFile = useHelmCodeProjectFileState(
  ...
  const importFileScript = useCallback(
    async (fileScript: HelmCodeProjectFileScript) => {
    ...
    <>
      <SettingsPageContainer>
        ...
        <SettingsSection title="New threads">
          ...
          <SettingsRow
            title="Workspace"
            description="Where new threads in this project start. Overrides helmcode.json and the global default; applies to every checkout in this group."
            resetAction={
              storedEnvMode !== null ? (
                <SettingResetButton
              ...
              >
                ...
                <SelectPopup align="end" alignItemWithTrigger={false}>
                  <SelectItem value="inherit">
                    {group.memberProjects.length > 1
                      ? "Default (each checkout's helmcode.json or global setting)"
        ...
        >
              ...
              {importableScripts.length > 0 ? (
                <Menu>
                  ...
                  <MenuPopup align="end" className="w-72">
                    <MenuGroup>
                      <MenuGroupLabel>Import from helmcode.json</MenuGroupLabel>
          ...
          {helmcodeFile.status === "invalid" ? (
            <SettingsRow
              title="helmcode.json is invalid"
              description="A helmcode.json exists in this checkout but fails to parse, so every action and icon it declares is ignored. Check the JSON syntax and icon values."
