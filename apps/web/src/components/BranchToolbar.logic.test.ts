import { EnvironmentId, type VcsRef } from "@helmcode/contracts";
  ...
  it("picks the most recently updated worktree thread", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "helmcode/older",
            worktreePath: "/repo/.helmcode/worktrees/older",
            updatedAt: "2026-07-20T00:00:00.000Z",
          },
          {
            branch: "helmcode/newer",
            worktreePath: "/repo/.helmcode/worktrees/newer",
    ...
    ).toEqual({ branch: "helmcode/newer", worktreePath: "/repo/.helmcode/worktrees/newer" });
  });
  ...
  it("skips the worktree the composer already points at", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "helmcode/current",
            worktreePath: "/repo/.helmcode/worktrees/current",
            updatedAt: "2026-07-22T00:00:00.000Z",
          },
        ],
        currentWorktreePath: "/repo/.helmcode/worktrees/current",
      }),
    ).toBeNull();
  });
  ...
  it("ignores archived threads and threads with unparseable timestamps", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "helmcode/archived",
            worktreePath: "/repo/.helmcode/worktrees/archived",
            updatedAt: "2026-07-23T00:00:00.000Z",
            archivedAt: "2026-07-23T01:00:00.000Z",
          },
          {
            branch: "helmcode/garbage-timestamp",
            worktreePath: "/repo/.helmcode/worktrees/garbage",
            updatedAt: "not-a-date",
          },
          {
            branch: "helmcode/live",
            worktreePath: "/repo/.helmcode/worktrees/live",
    ...
    ).toEqual({ branch: "helmcode/live", worktreePath: "/repo/.helmcode/worktrees/live" });
  });
});
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.helmcode/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a ref already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.helmcode/worktrees/feature-a",
        currentWorktreePath: null,
...
describe("resolveLocalCheckoutBranchMismatch", () => {
  ...
  it("ignores dedicated worktrees because their checkout is already thread-scoped", () => {
    expect(
      resolveLocalCheckoutBranchMismatch({
        effectiveEnvMode: "worktree",
        activeWorktreePath: "/repo/.helmcode/worktrees/feature-thread",
        activeThreadBranch: "feature/thread",
        currentGitBranch: "feature/current",
      }),
    ).toBeNull();
...
describe("resolveEffectiveEnvMode", () => {
  it("treats draft threads already attached to a worktree as current-checkout mode", () => {
    expect(
      resolveEffectiveEnvMode({
        activeWorktreePath: "/repo/.helmcode/worktrees/feature-a",
...
describe("resolveLockedWorkspaceLabel", () => {
  ...
  it("uses a shorter label for an attached worktree", () => {
    expect(resolveLockedWorkspaceLabel("/repo/.helmcode/worktrees/feature-a")).toBe("Worktree");
  });
});
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected ref", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        refName: {
          isDefault: false,
          worktreePath: "/repo/.helmcode/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.helmcode/worktrees/feature-b",
      nextWorktreePath: "/repo/.helmcode/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default refs", () => {
    expect(
      ...
      checkoutCwd: "/repo/.helmcode/worktrees/feature-a",
      nextWorktreePath: "/repo/.helmcode/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
