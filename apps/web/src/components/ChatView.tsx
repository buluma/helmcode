...
import {
  ...
  effectiveSnoozed,
  threadWokeAt,
} from "@helmcode/client-runtime/state/thread-settled";
import {
...
import { CHAT_LIST_ANCHOR_OFFSET } from "@helmcode/shared/chatList";
import { projectScriptCwd, projectScriptRuntimeEnv } from "@helmcode/shared/projectScripts";
import { truncate } from "@helmcode/shared/String";
import { nextTerminalId, resolveTerminalSessionLabel } from "@helmcode/shared/terminalLabels";
...
import { buildTemporaryWorktreeBranchName } from "@helmcode/shared/git";
...
} from "@helmcode/client-runtime/state/subagentRuntime";
...
import {
  ...
  threadHasOlderTurns,
} from "@helmcode/client-runtime/state/threads";
...
function ChatViewContent(props: ChatViewProps) {
  ...
  // the sent message can stay near the top. HelmCode only owns streaming adjustments
  ...
  const onSend = async (
    ...
    const composerImages =
      ...
      !sendContextImages.some((image) => image.id === directAnnotation.image?.id)
        ...
        : sendContextImages;
    const composerFiles = sendContextFiles;
    ...
    // Files can only ever go through the upload queue (the inline dataUrl
    // path only decodes images server-side), and addComposerAttachments
    // already refuses a file when uploads aren't supported — so any file
    // here means uploads are supported and every attachment, images
    // included, already has a server-side copy to reference by id.
    const turnAttachmentsPromise: Promise<Array<UploadChatAttachment | ChatAttachment>> =
      composerFilesSnapshot.length > 0 || supportsAttachmentUploads
        ? (async () => {
            const uploaded = getUploadedAttachments({
              environmentId,
              images: [...composerImagesSnapshot, ...composerFilesSnapshot],
            });
            if (!uploaded) {
              throw new Error("Attachment uploads are not finished yet.");
            }
            return uploaded;
          })()
        : Promise.all(
  ...
  const rightPanelContent = activeThreadRef ? (
    activeRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Helm Code server to browse pull requests."
      />
