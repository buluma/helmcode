...
export const HELMCODE_CHAT_THEME_ID = "helmcode-chat" as const;
export const HELMCODE_CHAT_THEME_LABEL = "Helm Code Chat";
...
export const CUSTOM_THEMES_STORAGE_KEY = "helmcode:themes:v1";
export const THEME_FOLLOW_SYSTEM_STORAGE_KEY = "helmcode:theme-follow-system";
export const THEME_APPEARANCE_MODE_STORAGE_KEY = "helmcode:theme-appearance-mode";
export const THEME_HALVES_STORAGE_KEY = "helmcode:theme-halves:v1";

const LEGACY_HELMCODE_CHAT_DARK_THEME_ID = "helmcode-chat-dark";
...
const RESERVED_THEME_IDS = new Set([
  ...
  "light",
  "dark",
  HELMCODE_CHAT_THEME_ID,
  GROVE_THEME_ID,
  OCEAN_THEME_ID,
  EMBER_THEME_ID,
  IRIS_THEME_ID,
  LEGACY_HELMCODE_CHAT_DARK_THEME_ID,
  "helmcode-grove",
  "helmcode-ocean",
  "helmcode-ember",
  "helmcode-iris",
...
// Earlier builds shipped every maintainer theme under a helmcode- prefix; only the
// genuinely HelmCode-branded palette keeps it. Stored preferences and mixes with the
// old ids stay readable through this alias table.
const LEGACY_THEME_ID_ALIASES: Readonly<Record<string, string>> = {
  [LEGACY_HELMCODE_CHAT_DARK_THEME_ID]: HELMCODE_CHAT_THEME_ID,
  "helmcode-grove": GROVE_THEME_ID,
  "helmcode-ocean": OCEAN_THEME_ID,
  "helmcode-ember": EMBER_THEME_ID,
  "helmcode-iris": IRIS_THEME_ID,
};
...
export function canonicalThemePreference(theme: string): string {
  return theme === LEGACY_HELMCODE_CHAT_DARK_THEME_ID ? theme : normalizeThemeId(theme);
}
...
// Older builds stored the dark Helm Code Chat palette as a separate theme. Keep
...
function legacyThemeMode(theme: ThemePreference): ThemeAppearance | null {
  return theme === LEGACY_HELMCODE_CHAT_DARK_THEME_ID ? "dark" : null;
}
...
// pixels users see after Helm Code Chat's blur and noise layers are composited.
// Foreground pairs deviate where necessary to keep normal text at WCAG AA.
const HELMCODE_CHAT_LIGHT_COLORS: ThemeColors = {
  canvas: "#fdf7fd",
  // Helm Code's workspace header belongs to the chat panel, so keep it seamless
  // with the light chat canvas rather than mapping it to Helm Code Chat's outer shell.
  chrome: "#fdf7fd",
  toolbar: "#fdf7fd",
  toolbarForeground: "#501854",
  toolbarBorder: "#efbdeb",
  // Helm Code Chat's light chrome controls sit on its pale gradient-noise surface,
  ...
  // Helm Code Chat uses a light lavender code surface in light mode. Keeping the
  // dark plum pair here also leaked the dark palette into Helm Code's diffs.
  codeBackground: "#f5ecf9",
  codeForeground: "#673c8b",
  // The live sidebar is transparent over Helm Code Chat's outer shell. Use that
...
const HELMCODE_CHAT_DARK_COLORS: ThemeColors = {
  canvas: "#1f1a24",
  // Helm Code's workspace header belongs to the chat panel, so keep it seamless
  // with the canvas rather than mapping it to Helm Code Chat's outer shell.
  chrome: "#1f1a24",
  toolbar: "#1f1a24",
  ...
  // Cards and panels stay in Helm Code Chat's plum surface family. Near-black here
  ...
  // Diffs and file previews are full workspace surfaces in Helm Code. Keep them
  ...
  // Helm Code Chat draws the chat panel edge in this muted pink. The resize rail uses
...
/**
 * The palette Helm Code wears with no theme installed, captured from the app's
 ...
 */
const HELM_CODE_LIGHT_THEME_COLORS: ThemeColors = {
  canvas: "#fcfcfc",
  chrome: "#fcfcfc",
  toolbar: "#fcfcfc",
...
const HELM_CODE_DARK_THEME_COLORS: ThemeColors = {
  canvas: "#0a0a0a",
  chrome: "#0a0a0a",
  toolbar: "#0a0a0a",
...
/**
 * The standard Helm Code look as a theme palette, for seeding a new theme when
 * no theme is installed. Distinct from {@link getDefaultThemeColors}, which
 * carries the flagship Helm Code Chat palette used to fill roles omitted by theme
 * files.
 */
export function getStandardThemeColors(appearance: ThemeAppearance): ThemeColors {
  if (appearance === "dark") {
    return (standardDarkThemeColors ??= decodeThemeColors(HELM_CODE_DARK_THEME_COLORS));
  }
  return (standardLightThemeColors ??= decodeThemeColors(HELM_CODE_LIGHT_THEME_COLORS));
}
...
/**
 * The status colors Helm Code shows without a theme, read from the app's own
...
export const HELMCODE_CHAT_THEME: ThemeDefinition = {
  id: HELMCODE_CHAT_THEME_ID,
  label: HELMCODE_CHAT_THEME_LABEL,
  appearance: "light",
  colors: decodeThemeColors(HELMCODE_CHAT_LIGHT_COLORS),
  variants: {
    dark: decodeThemeColors(HELMCODE_CHAT_DARK_COLORS),
  },
  sidebarArtwork: true,
};
...
export function getDefaultThemeColors(appearance: ThemeAppearance): ThemeColors {
  return appearance === "dark" ? HELMCODE_CHAT_THEME.variants!.dark! : HELMCODE_CHAT_THEME.colors;
}

/**
 * A companion action color in the Helm Code Chat mold. This gives send buttons,
