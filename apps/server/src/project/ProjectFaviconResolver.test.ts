...
import * as HelmCodeProjectFileLoader from "./HelmCodeProjectFileLoader.ts";
...
const makeTempDir = Effect.gen(function* () {
  ...
  return yield* fileSystem.makeTempDirectoryScoped({
    prefix: "helmcode-project-favicon-",
  });
});
...
const makeResolverWithFileSystem = (fileSystem: FileSystem.FileSystem) =>
  ProjectFaviconResolver.make.pipe(
    Effect.provide([WorkspacePaths.layer, HelmCodeProjectFileLoader.layer]),
  ...
  describe("resolvePath", () => {
    it.effect("serves repeated resolves from cache instead of re-walking candidates", () =>
      Effect.gen(function* () {
        ...
        expect(resolved).toBe(path.join(cwd, "public", "favicon.svg"));
    ...
    it.effect("prefers a helmcode.json iconPath over well-known files", () =>
      Effect.gen(function* () {
        const resolver = yield* ProjectFaviconResolver.ProjectFaviconResolver;
        const cwd = yield* makeTempDir;
        yield* writeTextFile(cwd, "helmcode.json", '{ "iconPath": "brand/mark.svg" }');
    ...
    it.effect("falls back to well-known files when the helmcode.json iconPath does not exist", () =>
      Effect.gen(function* () {
        const resolver = yield* ProjectFaviconResolver.ProjectFaviconResolver;
        const cwd = yield* makeTempDir;
        yield* writeTextFile(cwd, "helmcode.json", '{ "iconPath": "brand/missing.svg" }');
        yield* writeTextFile(cwd, "favicon.svg", "<svg>favicon</svg>");
    ...
    it.effect("ignores invalid helmcode.json files", () =>
      Effect.gen(function* () {
        const resolver = yield* ProjectFaviconResolver.ProjectFaviconResolver;
        const cwd = yield* makeTempDir;
        yield* writeTextFile(cwd, "helmcode.json", "{ not json");
        yield* writeTextFile(cwd, "favicon.svg", "<svg>favicon</svg>");
    ...
    it.effect("does not resolve a helmcode.json iconPath outside the workspace root", () =>
      Effect.gen(function* () {
        const resolver = yield* ProjectFaviconResolver.ProjectFaviconResolver;
        const parent = yield* makeTempDir;
        const cwd = `${parent}/app`;
        yield* writeTextFile(parent, "secret.svg", "<svg>secret</svg>");
        yield* writeTextFile(cwd, "helmcode.json", '{ "iconPath": "../secret.svg" }');
