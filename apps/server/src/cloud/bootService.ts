import { HostProcessExecutablePath, HostProcessPlatform } from "@helmcode/shared/hostProcess";
...
const BOOT_SERVICE_NAME = "helmcode";
export const BOOT_SERVICE_UNIT_FILE = `${BOOT_SERVICE_NAME}.service`;
export const BOOT_SERVICE_UNIT_ENV = "HELMCODE_BOOT_SERVICE_UNIT";
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=HELMCODE_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the Helm Code background service.";
  }
}
            ...
            Effect.mapError(
              (cause) =>
                new PinnedRuntimeInstallError({
                  step: "verifying the pinned helmcode runtime",
                  cause,
                }),
            ),
