...
import {
  ...
  signRelayJwt,
  verifyRelayJwt,
} from "@helmcode/shared/relayJwt";
import { isSecureRelayUrl } from "@helmcode/shared/relayUrl";
  ...
  Effect.mapError(
    () =>
      new EnvironmentHttpInternalServerError({
        message: "HELMCODE_RELAY_URL must be configured as a secure absolute HTTPS origin.",
      }),
  ),
);
...
const makeCloudLinkProof = Effect.fn("environment.cloud.makeLinkProof")(function* (
  ...
  const payload = {
    iss: `helmcode-env:${descriptor.environmentId}`,
...
const relayClientRequest = <A>(
  ...
  HttpClientRequest.post(input.url).pipe(
    ...
    Effect.mapError(
      (cause) =>
        new EnvironmentHttpInternalServerError({
          message: `HelmCode Connect relay request failed: ${String(cause)}`,
        }),
    ),
...
const reconcileDesiredCloudLinkWith = Effect.fn("environment.cloud.reconcileDesiredLinkWith")(
          ...
          onNone: () =>
            Effect.fail(
              new EnvironmentHttpUnauthorizedError({
                message: "Run `helmcode connect link` to authorize this environment.",
              }),
            ),
  ...
  Effect.catchIf(
    ServerSecretStore.isSecretStoreError,
    failEnvironmentCloudInternalError("Could not persist desired HelmCode Connect link state."),
  ),
  Effect.catchTags({
...
// explicit launcher stop (`helmcode service uninstall`, `systemctl stop`) during
...
export const releaseManagedTunnelOnShutdown = Effect.fn(
  ...
  // persisted URL: HELMCODE_RELAY_URL may have changed since the link was made.
...
const cloudEnvironmentHealthHandler = Effect.fn("environment.cloud.health")(
  function* (dependencies: CloudHttpDependencies, request: RelayCloudEnvironmentHealthRequest) {
    ...
    const proofOption = yield* verifyRelayJwt({
      ...
      audience: `helmcode-env:${environmentId}`,
    ...
    const responsePayload = {
      iss: `helmcode-env:${environmentId}`,
...
const cloudMintCredentialHandler = Effect.fn("environment.cloud.mintCredential")(
  function* (dependencies: CloudHttpDependencies, request: RelayCloudMintCredentialRequest) {
    ...
    const issued = yield* dependencies.environmentAuth.createPairingLink({
      ...
      label: "HelmCode Connect connect",
...
export const connectHttpApiLayer = HttpApiBuilder.group(
  ...
  Effect.fnUntraced(function* (handlers) {
    ...
    return handlers
      ...
      .handle("helmcodeMintCredential", ({ payload }) =>
