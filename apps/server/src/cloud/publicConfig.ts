...
} from "@helmcode/shared/connectAuth";
import { clerkFrontendApiUrlFromPublishableKey } from "@helmcode/shared/relayAuth";
import { normalizeSecureRelayUrl } from "@helmcode/shared/relayUrl";
...
declare const __HELMCODE_BUILD_RELAY_URL__: string | undefined;
declare const __HELMCODE_BUILD_CLERK_PUBLISHABLE_KEY__: string | undefined;
declare const __HELMCODE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: string | undefined;
declare const __HELMCODE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: string | undefined;
declare const __HELMCODE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: string | undefined;
declare const __HELMCODE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: string | undefined;
...
export const buildTimeRelayUrl =
  typeof __HELMCODE_BUILD_RELAY_URL__ === "undefined"
    ? ""
    : (normalizeSecureRelayUrl(__HELMCODE_BUILD_RELAY_URL__) ?? "");
export const buildTimeClerkPublishableKey = readBuildTimeValue(
  typeof __HELMCODE_BUILD_CLERK_PUBLISHABLE_KEY__ === "undefined"
    ? undefined
    : __HELMCODE_BUILD_CLERK_PUBLISHABLE_KEY__,
);
export const buildTimeClerkCliOAuthClientId = readBuildTimeValue(
  typeof __HELMCODE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__ === "undefined"
    ? undefined
    : __HELMCODE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__,
);
export const buildTimeRelayClientTracing = {
  tracesUrl: readBuildTimeValue(
    typeof __HELMCODE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__ === "undefined"
      ? undefined
      : __HELMCODE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__,
  ),
  tracesDataset: readBuildTimeValue(
    typeof __HELMCODE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__ === "undefined"
      ? undefined
      : __HELMCODE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__,
  ),
  tracesToken: readBuildTimeValue(
    typeof __HELMCODE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__ === "undefined"
      ? undefined
      : __HELMCODE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__,
  ),
} as const;
...
export function resolveRelayClientTracingConfig(
  ...
  const tracesUrl = env.HELMCODE_RELAY_CLIENT_OTLP_TRACES_URL?.trim() || fallback.tracesUrl;
  const tracesDataset =
    env.HELMCODE_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() || fallback.tracesDataset;
  const tracesToken = env.HELMCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() || fallback.tracesToken;
...
export function makeRelayUrlConfig(fallback = buildTimeRelayUrl) {
  const runtimeConfig = Config.nonEmptyString("HELMCODE_RELAY_URL");
...
export const hasCloudPublicConfig = Boolean(
  (normalizeSecureRelayUrl(process.env.HELMCODE_RELAY_URL ?? "") ?? buildTimeRelayUrl) &&
  (process.env.HELMCODE_CLERK_PUBLISHABLE_KEY?.trim() || buildTimeClerkPublishableKey) &&
  (process.env.HELMCODE_CLERK_CLI_OAUTH_CLIENT_ID?.trim() || buildTimeClerkCliOAuthClientId),
);
