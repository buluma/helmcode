...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    launcherPath: "/home/theo/.helmcode/runtime/service-launcher.mjs",
    baseDir: "/home/theo/.helmcode",
    logPath: "/home/theo/.helmcode/userdata/logs/boot-service.log",
    unitPath: "/home/theo/.config/systemd/user/helmcode.service",
  });

  expect(unit).toContain("ExecStart=/usr/bin/node /home/theo/.helmcode/runtime/service-launcher.mjs");
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const home = yield* fs.makeTempDirectoryScoped({ prefix: "helmcode-boot-service-test-" });
  const baseDir = path.join(home, ".helmcode");
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: (input) =>
      Effect.sync(() => {
        ...
        return {
          stdout: input.args[1] === "--version" ? "helmcode v1.2.3\n" : "",
          stderr: "",
  ...
  it.effect("restarts an installed service when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("systemctl "))).toEqual([
        "systemctl --user stop helmcode.service",
        "systemctl --user daemon-reload",
        "systemctl --user restart helmcode.service",
      ]);
    }),
  );
