...
import { SpawnExecutableResolution } from "@helmcode/shared/shell";
...
it.effect("launches an installed editor with platform-safe arguments", () =>
  Effect.gen(function* () {
    ...
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "helmcode-editors-" });
      ...
      Layer.mergeAll(
        ...
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: {
              PATH: "C:\\helmcode-editor-discovery-cache-test",
    ...
    Effect.provide(
      Layer.mergeAll(
        ...
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: {
              PATH: "C:\\helmcode-editor-discovery-interrupt-test",
              PATHEXT: ".COM;.EXE;.BAT;.CMD",
            },
          }),
        ),
