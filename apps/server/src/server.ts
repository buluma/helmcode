import { EnvironmentHttpApi } from "@helmcode/contracts";
...
import { disableTailscaleServe, ensureTailscaleServe } from "@helmcode/tailscale";
import { forkParked, ServerActivation } from "./serverActivation.ts";

// Effect's default preemptive shutdown waits 20s before finalizing request scopes.
// HelmCode's primary transport is long-lived WebSocket RPC, whose Effect scope finalizer
        ...
        yield* forkParked(
          Effect.gen(function* () {
            ...
            yield* reconcileDesiredCloudLink(`http://127.0.0.1:${address.port}`).pipe(
              ...
              Effect.tap(() => Effect.logInfo("HelmCode Connect desired link reconciled on startup")),
              Effect.catch((cause) =>
                Effect.logWarning("Failed to reconcile HelmCode Connect desired link on startup", {
                  cause,
                }),
              ),
            );
