...
export interface TelemetryCounters {
  ...
  readonly allHelmCode: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allHelmCode: ResourceTelemetryAggregate;
  };
...
export const emptyTelemetryCounters = (): TelemetryCounters => ({
  ...
  allHelmCode: emptyGroupCounters(),
});
...
function applyLifecycleCounters(input: {
  ...
  let allHelmCode = input.counters.allHelmCode;
  for (const processDelta of input.deltas) {
    ...
    allHelmCode = incrementCounters(allHelmCode, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allHelmCode = incrementCounters(allHelmCode, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allHelmCode = incrementCounters(allHelmCode, { processExits: 1 });
  }

  return { backend, electron, monitor, allHelmCode };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allHelmCode: aggregate(ordered, counters.allHelmCode),
    },
    deltas: processDeltas,
  };
}
