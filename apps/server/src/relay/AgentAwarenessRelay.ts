...
import { projectThreadAwareness } from "@helmcode/shared/agentAwareness";
import { makeDrainableWorker } from "@helmcode/shared/DrainableWorker";
...
>()("helmcode/relay/AgentAwarenessRelay") {}
...
const makePublishProof = Effect.fn("makePublishProof")(function* (input: {
  ...
  const payload = {
    iss: `helmcode-env:${input.environmentId}`,
      ...
      switch (startupState) {
        case "waiting-for-link":
          yield* Effect.logInfo(
            "agent activity publishing standby; waiting for HelmCode Connect link reconciliation",
          );
          break;
        case "disabled":
          yield* Effect.logInfo(
