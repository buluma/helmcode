...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@helmcode/contracts";
import { fromLenientJson } from "@helmcode/shared/schemaJson";
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".helmcode", "vcs.json");
