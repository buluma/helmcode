...
import { dedupeRemoteBranchesWithLocalMatches, normalizeGitRemoteUrl } from "@helmcode/shared/git";
import { compactTraceAttributes } from "@helmcode/shared/observability";
import { decodeJsonResult } from "@helmcode/shared/schemaJson";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `helmcode-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
    ...
    Effect.fn("refreshCheckedOutBranch")(function* (input) {
      ...
      if (!isAncestor) {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["update-ref", "refs/helmcode/pre-refresh", headCommit],
