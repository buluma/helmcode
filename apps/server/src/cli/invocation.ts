...
import { HostProcessArguments } from "@helmcode/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `helmcode` command.
 */
...
/**
 * The `helmcode` package spec to suggest. The literal spec the user typed (e.g.
 * `helmcode@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "helmcode@nightly" : "helmcode";
}

/**
 * Render a `helmcode <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx helmcode connect` suggests
 * `npx helmcode serve`, a global install suggests `helmcode serve`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `helmcode ${input.subcommand}`;
  }
