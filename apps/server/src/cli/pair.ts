/**
 * `helmcode pair` - mint a pairing token for an already-running server and print it
 ...
 * own `.helmcode` is checked first (matching dev-runner precedence); otherwise the
 * shared HelmCode home. `--tailscale` publishes the server over Tailscale Serve
...
import { resolveWorktreeHelmCodeHome } from "@helmcode/shared/devHome";
import {
  buildTailscaleHttpsBaseUrl,
  DEFAULT_TAILSCALE_SERVE_PORT,
  ensureTailscaleServe,
  readTailscaleStatus,
} from "@helmcode/tailscale";
...
const WELL_KNOWN_ENVIRONMENT_PATH = "/.well-known/helmcode/environment";
...
export class NoRunningServerError extends Schema.TaggedErrorClass<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      "No running Helm Code server found.",
      ...this.checkedStatePaths.map((statePath) => `  checked ${statePath}`),
      "Start one with `npx helmcode serve`, or connect this machine with HelmCode Connect: `npx helmcode connect`.",
    ].join("\n");
  }
}
...
export class ServesOtherEnvironmentError extends Schema.TaggedErrorClass<ServesOtherEnvironmentError>()(
  ...
  override get message(): string {
    return `Tailscale Serve on HTTPS port ${String(this.servePort)} already fronts a different Helm Code server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
export class ServePortOccupiedError extends Schema.TaggedErrorClass<ServePortOccupiedError>()(
  ...
  override get message(): string {
    return `HTTPS port ${String(this.servePort)} on the tailnet already serves something that is not a Helm Code server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
/**
 * Three outcomes, because they drive different decisions: a HelmCode descriptor
 * (pair with it), nothing answering (safe to configure Tailscale Serve), or
 * something answering that is not a HelmCode server (do NOT overwrite its mapping).
 */
type EnvironmentProbeResult =
  | { readonly _tag: "descriptor"; readonly descriptor: ExecutionEnvironmentDescriptor }
  | { readonly _tag: "unreachable" }
  | { readonly _tag: "not-a-helmcode-server" };
...
const probeEnvironmentDescriptor = (
  ...
  Effect.gen(function* () {
    ...
    // it as unreachable lets `helmcode pair --tailscale` repair its own mapping
    ...
    const descriptor = yield* HttpClientResponse.filterStatusOk(response).pipe(
      ...
      Effect.mapError(() => ({ _tag: "not-a-helmcode-server" }) as const),
    );
...
const discoverPairTarget = Effect.fn("pair.discoverPairTarget")(function* (
  ...
  if (explicitBaseDir !== undefined && explicitBaseDir.trim().length > 0) {
    ...
    // Same precedence as dev-runner: inside a linked worktree its own `.helmcode`
    // outranks the shared home, so `helmcode pair` in a worktree pairs with the dev
    // server under test rather than the daily-driver install.
    const worktreeHome = yield* resolveWorktreeHelmCodeHome(process.cwd());
    if (worktreeHome !== undefined) {
      bases.push(worktreeHome);
    }
    const envHome = yield* Config.string("HELMCODE_HOME").pipe(Config.option);
...
const resolveTailscalePairingBase = Effect.fn("pair.resolveTailscalePairingBase")(
  function* (input: { readonly target: DiscoveredPairTarget; readonly servePort: number }) {
    ...
    // environment, is safe to (re)configure. Any other responder — HelmCode or not
    ...
    if (existing._tag === "not-a-helmcode-server") {
...
const mintPairingLink = Effect.fn("pair.mintPairingLink")(function* (input: {
  ...
  return yield* Effect.gen(function* () {
    ...
    return yield* environmentAuth.createPairingLink({
      ...
      label: Option.getOrElse(input.label, () => "helmcode pair"),
...
export const pairCommand = Command.make("pair", {
  ...
  Command.withDescription(
    "Mint a pairing token for a running Helm Code server and print it as a QR code.",
  ),
