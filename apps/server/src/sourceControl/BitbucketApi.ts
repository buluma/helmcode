...
import { detectSourceControlProviderFromRemoteUrl } from "@helmcode/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("HELMCODE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("HELMCODE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("HELMCODE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("HELMCODE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("helmcode/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `helmcode/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set HELMCODE_BITBUCKET_EMAIL and HELMCODE_BITBUCKET_API_TOKEN, or HELMCODE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
