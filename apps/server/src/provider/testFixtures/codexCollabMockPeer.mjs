...
// HELMCODE_CODEX_COLLAB_SCRIPT env var (a JSON file path) when the first turn
...
const script = JSON.parse(NodeFS.readFileSync(process.env.HELMCODE_CODEX_COLLAB_SCRIPT, "utf8"));
...
rl.on("line", (line) => {
  ...
  if (method === "initialize") {
    write({
      id,
      result: {
        userAgent: "helmcode-collab-mock/0.0.0",
        codexHome: "/tmp",
        platformFamily: "unix",
        platformOs: "linux",
      },
  ...
  if (method === "thread/resume") {
    if (script.recordRequests) {
      NodeFS.appendFileSync(
        `${process.env.HELMCODE_CODEX_COLLAB_SCRIPT}.requests`,
  ...
  if (method === "turn/interrupt") {
    ...
    NodeFS.appendFileSync(
      `${process.env.HELMCODE_CODEX_COLLAB_SCRIPT}.interrupts`,
