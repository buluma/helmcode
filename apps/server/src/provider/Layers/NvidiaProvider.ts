...
const NVIDIA_PRESENTATION = {
  displayName: "NVIDIA",
  showInteractionModeToggle: false,
  supportsMultiAgentWorkflow: false,
} as const;

const DEFAULT_NVIDIA_MODEL_CAPABILITIES: ModelCapabilities = createModelCapabilities({
  optionDescriptors: [],
});
...
function normalizeNvidiaErrorMessage(cause: unknown): string | undefined {
  const message =
    typeof cause === "object" &&
    cause !== null &&
    "message" in cause &&
...
function formatNvidiaProbeError(input: {
  cause: unknown;
  ...
  if (
    ...
    return {
      installed: true,
      message: `Couldn't reach NVIDIA NIM at ${input.serverUrl}. Check the base URL and network.`,
    };
  }

  return {
    installed: true,
    message: detail ?? "Failed to connect to NVIDIA NIM.",
  };
}
...
export const makePendingNvidiaProvider = (input: {
  ...
  Effect.gen(function* () {
    ...
    if (!input.enabled) {
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "warning",
          auth: { status: "unknown" },
          message: "NVIDIA is disabled in Helm Code settings.",
        },
      });
    }
    ...
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "warning",
        auth: { status: "unknown" },
        message: "NVIDIA provider status has not been checked in this session yet.",
      },
    });
  });
...
export const checkNvidiaProviderStatus = (
  ...
  Effect.gen(function* () {
    ...
    const fallback = (errorCause: unknown): ServerProviderDraft => {
      const failure = formatNvidiaProbeError({
        cause: errorCause,
        isExternalServer,
      ...
      return buildServerProvider({
        ...
        checkedAt,
        models: providerModelsFromSettings([], customModels, DEFAULT_NVIDIA_MODEL_CAPABILITIES),
        probe: {
      ...
      return buildServerProvider({
        ...
        probe: {
          ...
          message: isExternalServer
            ? "NVIDIA is disabled in Helm Code settings. A base URL is configured."
            : "NVIDIA is disabled in Helm Code settings.",
        },
      });
    }
    ...
    const probe = Effect.gen(function* () {
      ...
      if (parsedResult._tag === "Failure") {
        throw new NvidiaProbeError("Response body was not valid JSON.", parsedResult.cause);
      }
    ...
    const models = providerModelsFromSettings(
      flattenNvidiaModels(probeExit.value),
      customModels,
      DEFAULT_NVIDIA_MODEL_CAPABILITIES,
    );
