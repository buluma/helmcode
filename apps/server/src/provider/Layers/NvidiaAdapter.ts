...
export const makeNvidiaAdapter = Effect.fn("makeNvidiaAdapter")(function* (input: {
    ...
    Effect.gen(function* () {
      ...
      const work = Effect.gen(function* () {
        ...
        Effect.catchCause((cause) =>
          Effect.ignoreCause(
            Effect.logError("NVIDIA adapter turn failed", { cause }),
          ),
        ),
      );
  ...
  const interruptTurn: NvidiaAdapterShape["interruptTurn"] = (_threadId: ThreadId, _turnId?: TurnId) =>
    Effect.sync(() => {
      sessions.delete(_threadId);
    });

  const respondToRequest: NvidiaAdapterShape["respondToRequest"] = (_threadId: ThreadId, _requestId: string, _decision: string) =>
    Effect.fail(
  ...
  const respondToUserInput: NvidiaAdapterShape["respondToUserInput"] = (_threadId: ThreadId, _requestId: string, _answers: unknown) =>
    Effect.fail(
      new ProviderAdapterValidationError({
        provider: NVIDIA,
  ...
  const rollbackThread: NvidiaAdapterShape["rollbackThread"] = (threadId: ThreadId, numTurns: number) =>
    Effect.gen(function* () {
      const session = sessions.get(threadId);
      if (!session) {
        return yield* Effect.fail(
