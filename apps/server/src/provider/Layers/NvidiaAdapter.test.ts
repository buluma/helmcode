...
it.effect("retries a request that never got a response as a transient failure", () =>
  Effect.gen(function* () {
    const threadId = ThreadId.make("nvidia-hung-request");
    let attempts = 0;
    const adapter = yield* makeTestAdapter().pipe(
      Effect.provide(
        Layer.succeed(
...
it.effect("listSessions reports the runtimeMode a session was actually started with", () =>
  Effect.gen(function* () {
    const threadId = ThreadId.make("nvidia-list-sessions-runtime-mode");
    const adapter = yield* makeTestAdapter().pipe(
      Effect.provide(testLayer(() => ({ body: chatCompletion("unused") }))),
    );
...
it.effect("interruptTurn actually stops the in-flight turn, not just its bookkeeping", () =>
  Effect.gen(function* () {
    const threadId = ThreadId.make("nvidia-interrupt-in-flight");
...
it.effect("rollbackThread returns the turns still present after a partial rollback", () =>
  Effect.gen(function* () {
    const threadId = ThreadId.make("nvidia-rollback-partial");
    let replyCount = 0;
    const adapter = yield* makeTestAdapter().pipe(
      Effect.provide(
        testLayer(() => {
...
it.effect("runs a workspace tool call before answering when a cwd is set", () =>
  Effect.gen(function* () {
    ...
    const threadId = ThreadId.make("nvidia-tool-call");
    const requests: CapturedRequest[] = [];
    const adapter = yield* makeTestAdapter().pipe(
      Effect.provide(
        testLayer((captured) => {
...
it.effect("rejects a read_file path that escapes the project root", () =>
  Effect.gen(function* () {
    ...
    const threadId = ThreadId.make("nvidia-tool-path-escape");
    const requests: CapturedRequest[] = [];
    const adapter = yield* makeTestAdapter().pipe(
      Effect.provide(
        testLayer((captured) => {
...
it.effect("falls back to no tools when the model rejects the tools field", () =>
  Effect.gen(function* () {
    ...
    const threadId = ThreadId.make("nvidia-tools-unsupported");
    const requests: CapturedRequest[] = [];
    const adapter = yield* makeTestAdapter().pipe(
      Effect.provide(
        testLayer((captured) => {
