...
import { runSqliteState } from "./helmcode-sqlite-state.ts";
...
it.layer(NodeServices.layer)("helmcode-sqlite-state", (it) => {
  it.effect("reports each invalid SQL source with a specific error", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "helmcode-sqlite-state-input-" });
  ...
  it.effect("queries an isolated database through Effect SQL", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "helmcode-sqlite-state-query-" });
  ...
  it.effect("backs up isolated state before writes and refuses the shared home", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "helmcode-sqlite-state-exec-" });
      yield* createFixtureDatabase(baseDir);
      ...
      const aliasParent = yield* fs.makeTempDirectoryScoped({
        prefix: "helmcode-sqlite-state-alias-",
      });
