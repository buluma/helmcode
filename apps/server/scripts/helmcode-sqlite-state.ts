...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start HelmCode once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedErrorClass<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.helmcode database. Use an isolated --base-dir.";
  }
}
...
export const helmcodeSqliteStateCommand = Command.make(
  "helmcode-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit HelmCode base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated HelmCode SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(helmcodeSqliteStateCommand, { version: "0.0.0" }).pipe(
