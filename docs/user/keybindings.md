...
The same configuration lives in `~/.helmcode/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. Helm Code writes the built-in defaults into that file on
