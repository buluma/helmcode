...
This guide is for people who want to use more than one Codex account in Helm Code. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Helm Code](./install.md).
...
issue. Helm Code uploads the thread and Codex logs to OpenAI and shows a thread ID that you can copy
...
reports them. If Codex does not report either value, Helm Code leaves it out instead of using the
parent agent's settings.

## Approve access to other apps

When a Codex tool needs access to an app such as Safari, Helm Code shows the app name and asks for
...
- both accounts can see the same HelmCode/Codex sessions
...
In Helm Code Settings, name it something obvious:
...
In Helm Code Settings, add another Codex provider:
...
Helm Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Helm Code treats it
