...
Helm Code has one server-side observability model:
...
stdout/stderr at `~/.helmcode/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.helmcode/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.helmcode/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.helmcode/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.helmcode/dev/logs/server.trace.ndjson`.
...
```bash
export HELMCODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export HELMCODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export HELMCODE_OTLP_SERVICE_NAME=helmcode-local
```

Optional:

```bash
export HELMCODE_TRACE_MIN_LEVEL=Info
export HELMCODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `HELMCODE_OTLP_*`.

macOS app bundle example:

```bash
HELMCODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
HELMCODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
HELMCODE_OTLP_SERVICE_NAME=helmcode-desktop \
"/Applications/Helm Code.app/Contents/MacOS/Helm Code"
```
...
```bash
TRACE_FILE="${HELMCODE_HOME:-$HOME/.helmcode}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.helmcode/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.helmcode/dev/logs/server.trace.ndjson"
```
...
- service name such as `helmcode-local`, `helmcode-dev`, or `helmcode-desktop`
...
- `helmcode_rpc_request_duration`
- `helmcode_orchestration_command_duration`
- `helmcode_orchestration_command_ack_duration`
- `helmcode_provider_turn_duration`
- `helmcode_git_command_duration`

Counters tell you volume and failure rate:

- `helmcode_rpc_requests_total`
- `helmcode_orchestration_commands_total`
- `helmcode_provider_turns_total`
- `helmcode_git_commands_total`
...
`helmcode_orchestration_command_ack_duration` measures:
...
1. Check `helmcode_orchestration_command_ack_duration` by `commandType`.
...
- `HELMCODE_OTLP_TRACES_URL` was not set
...
- `HELMCODE_TRACE_FILE`: override trace file path
- `HELMCODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `HELMCODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `HELMCODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `HELMCODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `HELMCODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `HELMCODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `HELMCODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `HELMCODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `HELMCODE_OTLP_SERVICE_NAME`: service name, default `helmcode-server`
