...
- Reads the shared production HelmCode Connect relay URL and Clerk client configuration before packaging clients.
...
- Publishes the CLI package (`apps/server`, npm package `helmcode`) with OIDC trusted publishing from the same workflow file:
...
through `/__helmcode/channel` on the router domain so the user's channel cookie is
...
- Publishes the CLI package (`apps/server`, npm package `helmcode`) to the `nightly` npm dist-tag using the same nightly version.
- Does not commit version bumps back to `main`.

## Server self-update release invariant

Connected servers update to the client's exact version, not to an npm dist-tag. Every released
desktop or hosted client version must therefore have a matching `helmcode@<version>` package available on
...
For a release smoke test, confirm `npm view helmcode@<version> version` returns the expected version, then
...
shows the exact local `npx helmcode@<version> service update` command. Also test the manual or
...
script temporarily prepares the `helmcode` package, then runs `vp pm publish --filter helmcode ...` from the
repository root so workspace publish configuration is applied correctly.

Checklist:

1. Confirm npm org/user owns package `helmcode` (or rename package first if needed).
...
`v0.0.0-test.1`, classifies the run as the stable channel. It publishes `helmcode` with npm dist-tag
...
2. Create an explicit App ID for `com.helmcode.helmcode` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [HelmCode Connect Clerk Setup](../internals/helmcode-connect.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.helmcode.helmcode` and includes
    Associated Domains.
