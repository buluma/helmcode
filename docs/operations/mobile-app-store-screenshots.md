...
The screenshot harness runs the real mobile application against three disposable local HelmCode
...
the software keyboard, and supplies deterministic HelmCode Connect discovery rows to the real
...
1. Creates three temporary HelmCode base directories and starts a local server for each on an available
   port.
2. Creates Helm Code, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic Helm Code review diff.
...
ARM64 by default for local Apple Silicon development; CI sets `HELMCODE_SHOWCASE_ANDROID_ABI=x86_64` so the
...
`pnpm --filter @helmcode/mobile showcase` starts Expo on its normal port, so it is not compatible with
...
The Pending rows use the production offline outbox and point at the real Helm Code and React fixture
