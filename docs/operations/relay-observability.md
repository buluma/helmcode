...
- `helmcode-code-relay-traces-prod`, the OpenTelemetry trace dataset shared by the Worker, mobile app, and
  first-party relay clients
- `helmcode-code-relay-otel-ingest-prod`, the dataset-scoped Worker ingest token
- `helmcode-code-mobile-otel-ingest-prod`, the dataset-scoped mobile ingest token
- `helmcode-code-relay-client-otel-ingest-prod`, the dataset-scoped first-party relay-client ingest token
- `helmcode-code-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`helmcode-code-relay-traces-dev-julius` for a personal stage.
...
```apl
['helmcode-code-relay-traces-prod']
