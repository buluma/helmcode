...
HelmCode Connect uses one Clerk application for web, desktop, and mobile authentication. The relay verifies
two kinds of bearer credential: template JWTs generated from the `helmcode-relay` template with the shared
`helmcode-code-relay` audience, and Clerk OAuth tokens issued to the CLI. `verifyRelayClientBearerToken` in
...
[helmcode-code-connect-auth-flow.html](./helmcode-code-connect-auth-flow.html).

## Application Keys

HelmCode Connect is disabled in a fresh clone. To enable it for source builds against the production
...
```dotenv
HELMCODE_CLERK_PUBLISHABLE_KEY=<publishable key>
HELMCODE_CLERK_JWT_TEMPLATE=<JWT template name>
HELMCODE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
HELMCODE_RELAY_URL=https://relay.example.com
```
...
should set `HELMCODE_CLERK_PUBLISHABLE_KEY`, `HELMCODE_CLERK_JWT_TEMPLATE`,
`HELMCODE_CLERK_CLI_OAUTH_CLIENT_ID`, and `HELMCODE_RELAY_URL` before building. EAS preview and
production builds only need the Clerk publishable key, JWT template name, and relay URL in their EAS
environment.

When any client-facing public value is absent, cloud UI is omitted. The `helmcode connect` command group is
...
deployment defaults.
`vp run --filter helmcode-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `helmcode connect` commands authorize a headless environment with a separate Clerk OAuth application.
This uses an OAuth public client with PKCE, so the CLI stores no client secret.

In **Clerk Dashboard > OAuth applications**:

1. Create an OAuth application for the HelmCode CLI.
...
3. Add **both** allowed redirect URIs:
   ...
   - `https://app.t3.codes/connect/callback` for the hosted out-of-band flow. This is
     ...
     custom `HELMCODE_HOSTED_APP_URL` means `$HELMCODE_HOSTED_APP_URL/connect/callback` instead.
     Omitting it breaks headless and SSH authorization.
4. Enable the `openid`, `profile`, and `email` scopes.
5. Set `HELMCODE_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
```sh
helmcode connect            # default: onboarding
helmcode connect login
helmcode connect link       # --publish-only
helmcode connect status     # --json
helmcode connect publish    # --disable
helmcode connect unlink
helmcode connect logout
```

`helmcode serve` is a separate top-level command, not a connect subcommand.

`helmcode connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `helmcode connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running HelmCode server. The next `helmcode serve` or `helmcode start` reconciles the relay link and launches the
managed tunnel. `helmcode connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `helmcode connect link` can re-enable exposure without another browser flow. `helmcode connect
...
logout leaves it running; manage it with `helmcode service status`, `install`, `update`, and `uninstall`.
...
Set `HELMCODE_CLERK_JWT_TEMPLATE=helmcode-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=helmcode-code-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `HELMCODE_RELAY_URL` still
...
Local desktop development uses `helmcode-dev://app`, while packaged builds use `helmcode://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `helmcode-dev://app`; the production Clerk instance should only need
`helmcode://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["helmcode://app"]}'
```
...
The production macOS bundle ID is `com.helmcode.helmcode`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.helmcode.helmcode` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.helmcode.helmcode`.
...
HELMCODE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/helmcode.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
HELMCODE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `HELMCODE_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`HELMCODE_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `HELMCODE_PORT` set. Rebuild the signed app
...
HELMCODE_PORT=13773 \
  "/Applications/Helm Code (Alpha).app/Contents/MacOS/Helm Code (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Helm Code (Alpha).app"
codesign -d --entitlements :- "/Applications/Helm Code (Alpha).app"
```
...
Signed-in users manage HelmCode Connect under **Connections**. The settings sidebar also has dedicated
controls, rendered by `SettingsSidebarNav.tsx`: `HelmCodeConnectSidebarSignIn` in the footer shows a
**Sign in to HelmCode Connect** button while signed out, and `HelmCodeConnectSidebarAvatar` shows a Clerk
