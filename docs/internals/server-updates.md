...
The launcher is the only runtime writer of `service-state.json`. `helmcode service install` and
`helmcode service update` may replace the launcher and state while the unit is stopped. Server children
...
1. The active server installs `helmcode@<target>` into a unique staging directory.
...
```sh
npx helmcode@<version> service update
```
