...
HelmCode has one runtime boundary: a client talks to a HelmCode server over HTTP and WebSocket, and the server
...
└───────────────┬──────────────────────────────┘
                │ connects to one HelmCode server
┌───────────────▼──────────────────────────────┐
│ Execution environment = one HelmCode server        │
...
One running HelmCode server instance. It owns provider availability and auth, model availability, projects
...
| `RelayConnectionTarget`   | Managed HelmCode Connect relay tunnels.                                  |
...
Tailscale is the first provider, and HelmCode manages more than discovery. When `tailscaleServeEnabled` is
...
(`apps/server/src/server.ts`, using [`@helmcode/tailscale`](../../packages/tailscale/src/tailscale.ts)).
...
Access answers one question: how does the client speak WebSocket to a HelmCode server? It does not answer
how the server got started or who manages the process.

### Direct WebSocket access

`wss://helmcode.example.com` or `ws://10.0.0.15:3773`, paired as a bearer target. This is the base model.
...
Managed HelmCode Connect relay tunnels use `RelayConnectionTarget` and are the answer when the host is
...
Worker itself. See [helmcode-connect.md](./helmcode-connect.md).

### Tailscale access

A HelmCode-managed `tailscale serve` mapping exposes the server on the tailnet over HTTPS, and the
...
[packages/ssh/src/tunnel.ts][sshtunnel], which resolves the target, launches or reuses the remote HelmCode
...
Launch answers a different question: how does a HelmCode server come to exist on the target machine? Keep
it separate from access.

- **Pre-existing server.** The operator already runs HelmCode and the client connects directly or through a
  tunnel.
...
- **Client-managed local publish.** A local server is published through the relay with
  `helmcode connect link`, exposing a desktop-hosted environment to mobile without router or firewall
  changes.
...
- a relay-hosted OAuth callback broker (see [helmcode-connect.md](./helmcode-connect.md));
