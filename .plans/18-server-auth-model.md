...
Define the long-term server auth architecture for Helm Code before first-class remote environments ship.
...
Every privileged surface of the HelmCode server must go through the same auth policy engine:
...
Participants:
  ...
  HelmCodeServer      = local backend child process
...
DesktopMain -> HelmCodeServer : spawn server (--bootstrap-fd ...)
DesktopMain -> HelmCodeServer : send desktop bootstrap envelope
note over HelmCodeServer : policy = DesktopManagedLocalPolicy
note over HelmCodeServer : allowed pairing = desktop-bootstrap only

Frontend -> DesktopMain : request local bootstrap grant
DesktopMain --> Frontend : short-lived desktop bootstrap grant

Frontend -> HelmCodeServer : POST /api/auth/bootstrap
HelmCodeServer -> HelmCodeServer : validate desktop bootstrap grant
HelmCodeServer -> HelmCodeServer : create browser session
HelmCodeServer --> Frontend : Set-Cookie: session=...

Frontend -> HelmCodeServer : GET /ws + authenticated cookie
HelmCodeServer -> HelmCodeServer : validate cookie session
HelmCodeServer --> Frontend : websocket accepted
```
...
```text
Participants:
  UserShell     = npx helmcode launcher
  HelmCodeServer      = standalone local server
  Browser       = browser tab

UserShell -> HelmCodeServer : start server
HelmCodeServer -> HelmCodeServer : getOrCreate("server-signing-key")
note over HelmCodeServer : policy = LoopbackBrowserPolicy

UserShell -> HelmCodeServer : issue one-time pairing token
HelmCodeServer --> UserShell : pairing URL or pairing token

UserShell --> Browser : open /pair?token=...

Browser -> HelmCodeServer : GET /pair?token=...
HelmCodeServer -> HelmCodeServer : validate one-time token
HelmCodeServer -> HelmCodeServer : create browser session
HelmCodeServer --> Browser : Set-Cookie: session=...
HelmCodeServer --> Browser : redirect to app

Browser -> HelmCodeServer : GET /ws + authenticated cookie
HelmCodeServer --> Browser : websocket accepted
```
...
Participants:
  ...
  HelmCodeServer      = HelmCode server
  PhoneBrowser  = mobile browser

DesktopUser -> DesktopMain : enable remote access via tunnel
DesktopMain -> HelmCodeServer : switch policy to RemoteReachablePolicy
DesktopMain -> Tunnel : publish local HelmCode endpoint
Tunnel --> DesktopMain : public https/wss URL

DesktopMain -> HelmCodeServer : issue one-time pairing token
HelmCodeServer --> DesktopMain : pairing token
...
Tunnel -> HelmCodeServer : forward request
HelmCodeServer -> HelmCodeServer : validate one-time token
HelmCodeServer -> HelmCodeServer : create mobile browser session
HelmCodeServer --> PhoneBrowser : Set-Cookie: session=...
HelmCodeServer --> PhoneBrowser : redirect to app

PhoneBrowser -> Tunnel : GET /ws + authenticated cookie
Tunnel -> HelmCodeServer : forward websocket upgrade
HelmCodeServer --> PhoneBrowser : websocket accepted
```
...
DesktopUser -> HelmCodeServer : enable private-network access
HelmCodeServer -> HelmCodeServer : switch policy to RemoteReachablePolicy
DesktopUser -> HelmCodeServer : issue one-time pairing token
HelmCodeServer --> DesktopUser : pairing URL / QR

DesktopUser -> PhoneBrowser : open private-network URL
PhoneBrowser -> PrivateNet : GET http(s)://private-host/pair?token=...
PrivateNet -> HelmCodeServer : route request
HelmCodeServer -> HelmCodeServer : validate one-time token
HelmCodeServer -> HelmCodeServer : create mobile browser session
HelmCodeServer --> PhoneBrowser : Set-Cookie: session=...
HelmCodeServer --> PhoneBrowser : redirect to app
...
PrivateNet -> HelmCodeServer : websocket upgrade
HelmCodeServer --> PhoneBrowser : websocket accepted
```
...
Participants:
  ...
  RemoteT3      = remote HelmCode server
  Frontend      = desktop renderer

DesktopUser -> DesktopMain : add SSH host
DesktopMain -> SSH : connect to remote host
SSH -> RemoteHost : probe environment / verify helmcode availability
DesktopMain -> SSH : run remote launch command
SSH -> RemoteHost : helmcode remote launch --json
