...
describe("ssh tunnel scripts", () => {
  ...
  it("shell-quotes package specs in the remote helmcode runner", () => {
    ...
    assert.include(
      script,
      "exec npm exec --yes 'helmcode@nightly; touch /tmp/helmcode-owned' -- \"$@\"",
    );
  ...
  it("uses the remote helmcode runner for launch and pairing scripts", () => {
    ...
    assert.include(
      buildRemoteLaunchScript({ packageSpec: "helmcode@nightly" }),
      "helmcode@nightly",
    );
    assert.include(
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "helmcode@nightly" }),
      "helmcode@nightly",
    );
    assert.include(
