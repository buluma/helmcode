...
import {
  ...
  buildRemoteHelmCodeRunnerScript,
...
describe("ssh tunnel scripts", () => {
  it("builds the remote helmcode runner with npx and npm fallbacks", () => {
    const script = buildRemoteHelmCodeRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "HELMCODE_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec helmcode "$@"');
    assert.include(script, "exec npx --yes 'helmcode@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'helmcode@latest' -- \"$@\"");
    assert.include(script, "could not install 'helmcode@latest'");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `HELMCODE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for HELMCODE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteHelmCodeRunnerScript();

    assert.include(script, "HELMCODE_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote helmcode runner", () => {
    const script = buildRemoteHelmCodeRunnerScript({
      packageSpec: "helmcode@nightly; touch /tmp/helmcode-owned",
    });

    assert.include(script, "exec npx --yes 'helmcode@nightly; touch /tmp/helmcode-owned' \"$@\"");
    assert.include(script, "exec npm exec --yes 'helmcode@nightly; touch /tmp/helmcode-owned' -- \"$@\"");
    assert.notInclude(script, "exec npx --yes helmcode@nightly; touch /tmp/helmcode-owned");
  });

  it("builds the remote helmcode runner with a node script override", () => {
    const script = buildRemoteHelmCodeRunnerScript({
    ...
    assert.include(
      script,
      "HELMCODE_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$HELMCODE_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote helmcode runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `HELMCODE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote HelmCode server did not become ready");
    assert.include(buildRemoteLaunchScript({ packageSpec: "helmcode@nightly" }), "helmcode@nightly");
    assert.include(
    ...
    assert.include(buildRemotePairingScript(target, { packageSpec: "helmcode@nightly" }), "helmcode@nightly");
    assert.include(
