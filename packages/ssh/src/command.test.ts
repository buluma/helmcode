...
import {
  ...
  resolveRemoteHelmCodeCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote helmcode package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemoteHelmCodeCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "helmcode@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemoteHelmCodeCliPackageSpec({
