...
} from "@helmcode/shared/httpReadiness";
import * as NetService from "@helmcode/shared/Net";
import { extractJsonObject, fromLenientJson } from "@helmcode/shared/schemaJson";
import { satisfiesSemverRange } from "@helmcode/shared/semver";
...
export interface RemoteHelmCodeRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteHelmCodeRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteHelmCodeRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  HELMCODE_NODE_ENGINE_RANGE=@@HELMCODE_NODE_ENGINE_RANGE@@
  if [ -z "$HELMCODE_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$HELMCODE_NODE_ENGINE_RANGE" <<'NODE'
@@HELMCODE_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for HELMCODE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$HELMCODE_NODE_BIN/node" ]; then
        PATH="$HELMCODE_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@HELMCODE_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
HELMCODE_NODE_SCRIPT_PATH=@@HELMCODE_NODE_SCRIPT_PATH@@
if [ -n "$HELMCODE_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$HELMCODE_NODE_SCRIPT_PATH" "$@"
fi
if command -v helmcode >/dev/null 2>&1; then
  exec helmcode "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@HELMCODE_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@HELMCODE_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the helmcode CLI and could not install @@HELMCODE_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@HELMCODE_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.helmcode/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.helmcode"
...
RUNNER_NEXT="$STATE_DIR/run-helmcode.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@HELMCODE_DEFAULT_REMOTE_PORT@@" "@@HELMCODE_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@HELMCODE_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@HELMCODE_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@HELMCODE_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@HELMCODE_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@HELMCODE_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@HELMCODE_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env HELMCODE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@HELMCODE_READY_TIMEOUT_MS@@"; then
    printf 'Remote HelmCode server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.helmcode/ssh-launch/@@HELMCODE_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.helmcode"
RUNNER_FILE="$STATE_DIR/run-t3.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteHelmCodeRunnerScript(input?: RemoteHelmCodeRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "helmcode@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      HELMCODE_PACKAGE_SPEC: packageSpec,
      HELMCODE_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      HELMCODE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemoteHelmCodeRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      HELMCODE_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      HELMCODE_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteHelmCodeRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    HELMCODE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    HELMCODE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteHelmCodeRunnerScript(input)),
    HELMCODE_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    HELMCODE_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    HELMCODE_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    HELMCODE_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    HELMCODE_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    HELMCODE_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    HELMCODE_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteHelmCodeRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    HELMCODE_STATE_KEY: remoteStateKey(target),
    HELMCODE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteHelmCodeRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteHelmCodeRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteHelmCodeRunnerOptions;
...
>()("@helmcode/ssh/tunnel/SshEnvironmentManager") {
