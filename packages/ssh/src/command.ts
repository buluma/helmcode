...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@helmcode/contracts";
...
const PUBLISHABLE_HELMCODE_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteHelmCodeCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_HELMCODE_VERSION_PATTERN.test(appVersion)) {
    return `helmcode@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "helmcode@nightly";
  }

  return input.updateChannel === "nightly" ? "helmcode@nightly" : "helmcode@latest";
}
