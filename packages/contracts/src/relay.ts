...
}).annotate({ description: "Links an authenticated cloud user to a HelmCode environment." });
...
>()("@helmcode/contracts/relay/RelayClientPrincipal") {}
...
>()("@helmcode/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in HelmCode Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["helmcode-mobile", "helmcode-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "helmcode-mobile" as const;
export const RelayWebClientId = "helmcode-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Helm Code Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking HelmCode environments, connecting authorized clients, and publishing agent activity.",
  );
