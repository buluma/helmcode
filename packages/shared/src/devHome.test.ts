...
import { resolveGitWorktreePath, resolveWorktreeHelmCodeHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "helmcode-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeHelmCodeHome", () => {
  it.effect("answers with .helmcode before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeHelmCodeHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".helmcode"));
