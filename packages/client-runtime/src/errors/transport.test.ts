...
describe("isTransportConnectionErrorMessage", () => {
  ...
  it("returns true for the HelmCode server WebSocket message", () => {
    expect(isTransportConnectionErrorMessage("Unable to connect to the HelmCode server WebSocket.")).toBe(
      true,
    );
  });
