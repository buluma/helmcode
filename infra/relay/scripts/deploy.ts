...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    HELMCODE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    HELMCODE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    HELMCODE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    HELMCODE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    HELMCODE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    HELMCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("HELMCODE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("HELMCODE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "HelmCodeRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Helm Code relay through Alchemy."));
