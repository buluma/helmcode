...
describe("reconcileRootEnvRelayUrl", () => {
  ...
  it("preserves unrelated root env entries while replacing a previous relay URL", () => {
    expect(
      reconcileRootEnvRelayUrl(
        "HELMCODE_CLERK_PUBLISHABLE_KEY=pk_test_example\nHELMCODE_RELAY_URL=https://old.example.test\n",
...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "helmcode-code-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "helmcode-code-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "HELMCODE_MOBILE_OTLP_TRACES_DATASET=helmcode-code-mobile-traces-dev",
        "HELMCODE_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "HELMCODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "HELMCODE_RELAY_CLIENT_OTLP_TRACES_DATASET=helmcode-code-relay-client-traces-dev",
        "HELMCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
    ...
    expect(
      reconcileRootEnvPublicConfig(
        [
          "HELMCODE_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "HELMCODE_RELAY_URL=https://old.example.test",
          "HELMCODE_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "HELMCODE_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "HELMCODE_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "HELMCODE_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "HELMCODE_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "HELMCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "HELMCODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "HELMCODE_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "HELMCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
