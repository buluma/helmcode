...
import { RELAY_LINK_PROOF_TYP } from "@helmcode/shared/relayJwt";
...
const makeRequest = Effect.gen(function* () {
  ...
  const payload = {
    iss: "helmcode-env:env-link-test",
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.succeed(EnvironmentCredentials.EnvironmentCredentials, {
          create: () => Effect.succeed("helmcode_env_credential_secret"),
...
describe("EnvironmentLinker", () => {
  it.effect("uses verified JWT claims when linking an environment", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(result.environmentCredential).toBe("helmcode_env_credential_secret");
