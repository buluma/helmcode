...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("helmcode-code-relay-traces", "prod")).toBe(
      "helmcode-code-relay-traces-prod",
    );
    expect(relayResourceNameForStage("helmcode-code-relay-traces", "dev_julius")).toBe(
      "helmcode-code-relay-traces-dev-julius",
    );
  });
});
