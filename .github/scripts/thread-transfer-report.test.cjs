...
test("renders baseline, impact, ceiling, and ceiling changes", () => {
  ...
  const comment = renderComment({
    ...
    currentRun: {
      ...
      conclusion: "success",
      url: "https://github.com/buluma/helmcode/actions/runs/2",
    },
    baselineRun: {
      sha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
      matchesBase: true,
      url: "https://github.com/buluma/helmcode/actions/runs/1",
    },
  });
  ...
  assert.match(comment, /<!-- helmcode-thread-transfer-report -->/);
  assert.match(
    comment,
    /<!-- helmcode-thread-transfer-result-sha:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb -->/,
  );
});
...
test("resolves a fallback PR with a redacted head repo and exact main baseline", async () => {
  ...
  await resolve({
    github,
    context: {
      repo: { owner: "buluma", repo: "helmcode" },
      payload: {
        workflow_run: {
          id: 2,
          event: "pull_request",
          ...
          head_repository: { full_name: "buluma/helmcode" },
          conclusion: "success",
          pull_requests: [],
        },
      },
      ...
      paginate: async (method) => {
        ...
        return [5350, 5351].map((number) => ({
          ...
          head: {
            sha: "head-sha",
            ref: "feature-branch",
            repo: { full_name: "buluma/helmcode" },
          },
        }));
      },
      rest: {
      ...
      paginate: async () => [
        {
          ...
          body: `<!-- helmcode-thread-transfer-report -->\n<!-- helmcode-thread-transfer-result-sha:${sha} -->`,
        },
      ],
      rest: {
        issues: {
